# CLAUDE CODE CONFIGURATION - alanOS PROJECT

This file contains project-specific configuration and preferences for Claude Code when working with the alanOS codebase.

---

## PROJECT INFORMATION

- **Working Directory:** `/Users/{user}/Documents/GitHub/alan`
- **Git Repository:** Yes
- **Main Branch:** `develop`
...
alanOS is organized as a monorepo with the following key packages:

### Core Packages

- **`packages/core`** - `@alanos/core` - Foundation runtime, types, agents, and database
- **`packages/cli`** - `@alanos/cli` - Command-line interface and agent runtime
...
- **`packages/api-client`** - `@alanos/api-client` - Type-safe API client for alanOS server
...
- **`packages/create-alan`** - Project scaffolding tool
...
- Use `bun` for global installs: `bun install -g @alanos/cli`
...
- **The `alanos` CLI** is built from `packages/cli`
- **INTENDED FOR:** Production use by developers/users of the project
- **DO NOT USE THE `alanos` CLI WITHIN THE `alan` MONOREPO ITSELF**
- **The `alanos` CLI is for external consumers, NOT internal monorepo development**
- **For monorepo development:** Use `bun` commands directly

### alanOS Test Command

The `alanos test` command runs tests for alanOS projects and plugins:

```bash
alanos test [path]           # Run all tests (component + e2e)
alanos test -t component     # Run only component tests
alanos test -t e2e          # Run only e2e tests
alanos test --name "test"   # Filter tests by name
alanos test --skip-build    # Skip building before tests
```

**Test Types:**

- **Component Tests:** Unit tests via `bun test` - test individual modules/components in isolation
- **E2E Tests:** Full integration tests via alanOS TestRunner - test complete agent runtime with server, database, and plugins

**Context Support:**

- Works in both monorepo packages and standalone projects created with `alanos create`
- Automatically detects project type and adjusts paths accordingly
- For plugins: Creates default alan character as test agent
- For projects: Uses agents defined in project configuration

**Note:** The test command does NOT run Cypress or other UI tests - only alanOS-specific tests
...
- **Central Dependency:** Everything depends on `@alanos/core` or `packages/core`
- **No Circular Dependencies:** Core cannot depend on other packages
- **Import Pattern:** Use `@alanos/core` in package code, `packages/core` in internal references
...
### CRITICAL: alanOS Component Clarifications
...
3. **Import Errors:** Verify correct use of `@alanos/core` vs `packages/core`
...
_This configuration file should be referenced at the start of any alanOS development session to ensure proper setup and adherence to project standards._
