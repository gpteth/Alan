...
fn shutdown_server() {
    println!("Shutting down alan server...");
    let mut guard = SERVER_PROCESS.lock().unwrap();
    if let Some(ref mut child) = *guard {
        if let Err(e) = child.kill() {
            eprintln!("Failed to kill alan server: {}", e);
        } else {
            println!("alan server shut down successfully");
        }
    }
    *guard = None;
}
    ...
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if !is_server_running() {
                println!("Starting alan server...");
                match Command::new("alanos")
                    .arg("start")
                    .spawn() {
                        Ok(child) => {
                            ...
                            *server_guard = Some(child);
                            println!("alan server process started");
                        },
                        Err(e) => {
                            eprintln!("Failed to start alan server: {}", e);
                        }
                    };
            } else {
                println!("alan server is already running");
            }
