...
export class FullDocumentationGenerator {
  ...
  private async generateInstallation(packageJson: any): Promise<string> {
    ...
    try {
      ...
      const prompt = `Generate installation and integration instructions for this alanOS plugin:
            ...
            This is a plugin for the alanOS agent system. Generate comprehensive installation instructions that include:

            1. How to add the plugin to your alanOS project:
  ...
  private async generateBasicInstallPrompt(packageJson: any): Promise<string> {
    ...
    const prompt = `Generate basic installation instructions for this alanOS plugin:

        Plugin name: ${packageJson.name}
        Dependencies: ${JSON.stringify(packageJson.dependencies || {}, null, 2)}
        Peer dependencies: ${JSON.stringify(packageJson.peerDependencies || {}, null, 2)}

        This is a plugin for the alanOS agent system. Include basic setup instructions.`;
