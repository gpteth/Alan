...
# to generate alanOS plugins programmatically.
...
create_time_tracker_plugin() {
    ...
    if [ ! -f "package.json" ] || ! grep -q "@alanos/cli" "package.json"; then
...
async function createPlugin() {
    ...
    try {
        ...
        if (result.success) {
            ...
            console.log('5. Add to your alanOS project');
            process.exit(0);
        } else {
