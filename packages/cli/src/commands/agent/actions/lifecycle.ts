      ...
      if (errorMsg.startsWith('AGENT_NOT_FOUND:')) {
        ...
        try {
          ...
          if (agents.length > 0) {
            ...
            console.error(
              `\nYou can create a new agent with: alanos create -t agent ${agentName.toLowerCase()}`
            );
          }
        } catch (error) {
...
export async function stopAgent(opts: OptionValues): Promise<void> {
  try {
    ...
    if (!hasValidName && !opts.all) {
      ...
      console.error('  alanos agent stop --name alan');
      console.error('  alanos agent stop --all');
      process.exit(1);
    }

    // If --all flag is provided, stop all local alanOS processes
    if (opts.all) {
      logger.info('Stopping all alanOS agents...');
      ...
      try {
        ...
        // Support both node and bun executables, and look for common alanOS patterns
        const patterns = [
          '(node|bun).*alanos',
          '(node|bun).*alan.*start',
        ...
        logger.success('All alanOS agents stopped successfully!');
      } catch (error) {
        logger.error(
