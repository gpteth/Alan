...
export async function createPlugin(
  ...
  await withCleanupOnInterrupt(pluginTargetDir, pluginDirName, async () => {
    ...
    console.info(`\n  Common commands:`);
    console.info(`  alanos dev    # Start development mode with hot reloading`);
    console.info(`  alanos start  # Start in production mode`);
...
export async function createAgent(
  ...
  // Create agent character based on alan template
  ...
  console.info(`\nTo use this agent:`);
  console.info(`  1. Start alanOS server with this character:`);
  console.info(`     alanos start --character ${agentFilePath}`);
  console.info(`\n  OR if a server is already running:`);
  console.info(`     alanos agent start --path ${agentFilePath}`);
}
