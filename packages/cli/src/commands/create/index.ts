...
export const create = new Command('create')
  .description('Create a new alanOS project, plugin, agent, or TEE project')
  ...
  .action(async (name?: string, opts?: any) => {
    ...
    try {
      ...
      if (
        process.env.alan_NONINTERACTIVE === '1' ||
        process.env.alan_NONINTERACTIVE === 'true' ||
      ...
      if (!projectName) {
        if (!isNonInteractive) {
          const selectedType = await clack.select({
            ...
            options: [
              {
                label: 'Project - Full alanOS application',
                value: 'project',
                hint: 'Complete project with runtime, agents, and all features',
              },
              {
                label: 'Plugin - Custom alanOS plugin',
                value: 'plugin',
                hint: 'Extend alanOS functionality with custom plugins',
              },
              {
        ...
        if (!isNonInteractive) {
          ...
          clack.intro(colors.inverse(` Creating alanOS ${introType} `));
        }
        ...
        if (!isNonInteractive) {
          ...
          throw new Error(`Project name is required. Usage: alanos create [name]`);
        }
      }
