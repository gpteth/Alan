...
export async function startDevMode(options: DevOptions): Promise<void> {
  ...
  const isProject = directoryType.type === 'alanos-project';
  const isPlugin = directoryType.type === 'alanos-plugin';
  const isMonorepo = directoryType.type === 'alanos-monorepo';

  // Log project type
  if (isProject) {
    console.info('Identified as an alanOS project package');
  } else if (isPlugin) {
    console.info('Identified as an alanOS plugin package');
  } else if (isMonorepo) {
    console.info('Identified as an alanOS monorepo');
  } else {
    console.warn(
      `Not in a recognized alanOS project, plugin, or monorepo directory. Current directory is: ${directoryType.type}. Running in standalone mode.`
    );
  }
  ...
  if (process.env.alan_TEST_MODE === 'true') {
