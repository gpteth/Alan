...
function setupEnvironment(): Record<string, string> {
  ...
  // Preserve alan_TEST_MODE for test environments
  if (process.env.alan_TEST_MODE) {
    env.alan_TEST_MODE = process.env.alan_TEST_MODE;
  }
...
async function startServerProcess(args: string[] = []): Promise<void> {
  ...
  if (localCliPath) {
    console.info('Using local @alanos/cli installation');
  ...
  const isTestMode = process.env.alan_TEST_MODE === 'true';
