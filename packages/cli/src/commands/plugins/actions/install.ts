...
export async function installPluginFromRegistry(
  ...
  if (!cachedRegistry || !cachedRegistry.registry) {
    logger.error('Plugin registry cache not found. Please run "alanos plugins update" first.');
    process.exit(1);
  }
...
export async function addPlugin(pluginArg: string, opts: AddPluginOptions): Promise<void> {
  ...
  if (!pluginArg || !pluginArg.trim()) {
    ...
    logger.info(
      'Please provide a valid plugin name (e.g., "openai", "plugin-anthropic", "@alanos/plugin-sql")'
    );
    process.exit(1);
  }
  ...
  if (!directoryInfo || !directoryInfo.hasPackageJson) {
    logger.error(
      `Command must be run inside an alanOS project directory. This directory is: ${directoryInfo?.type || 'invalid or inaccessible'}`
    );
    process.exit(1);
  }
