...
export async function listAvailablePlugins(opts: ListPluginsOptions): Promise<void> {
  ...
  if (
    ...
    console.log('\nPlease run "alanos plugins update" to fetch the latest plugin registry.');
    return;
  }
...
export async function listInstalledPlugins(): Promise<void> {
  ...
  if (!directoryInfo || !directoryInfo.hasPackageJson) {
    ...
    console.info('Please run this command from the root of an alanOS project.');
    process.exit(1);
  }
  ...
  const pluginNames = Object.keys(allDependencies).filter((depName) => {
    return /^(@alanos(-plugins)?\/)?plugin-.+/.test(depName);
  });

  if (pluginNames.length === 0) {
    console.log('No alan plugins found in the project dependencies (package.json).');
  } else {
