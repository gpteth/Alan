...
export const extractPluginEnvRequirements = async (
  ...
  try {
    ...
    const possiblePaths = [
      ...
      // Scoped package path (e.g., @alanos/plugin-discord)
      path.join(cwd, 'node_modules', packageName.replace('/', path.sep), 'package.json'),
    ];

    // If the package name doesn't start with @alanos/, also try with that prefix
    if (!packageName.startsWith('@alanos/')) {
      possiblePaths.push(
        path.join(cwd, 'node_modules', '@alanos', packageName, 'package.json'),
        path.join(cwd, 'node_modules', '@alanos', `plugin-${packageName}`, 'package.json')
      );
    }
    ...
    if (!packageJsonPath) {
      ...
      if (existsSync(nodeModulesPath)) {
        ...
        for (const pkg of packages) {
          if (pkg.includes(packageName.replace('@alanos/', '').replace('plugin-', ''))) {
...
export const promptForPluginEnvVars = async (packageName: string, cwd: string): Promise<void> => {
  ...
  // If no requirements found and package doesn't start with @alanos/, try with that prefix
  if (Object.keys(envRequirements).length === 0 && !packageName.startsWith('@alanos/')) {
    // Try with @alanos/ prefix
    const alanosPackageName = `@alanos/${packageName.replace('plugin-', '')}`;
    envRequirements = await extractPluginEnvRequirements(alanosPackageName, cwd);

    // Also try with @alanos/plugin- prefix
    if (Object.keys(envRequirements).length === 0) {
      const alanosPluginPackageName = `@alanos/plugin-${packageName.replace('plugin-', '')}`;
      envRequirements = await extractPluginEnvRequirements(alanosPluginPackageName, cwd);
    }
  }
  ...
  if (Object.keys(envRequirements).length === 0) {
    ...
    const possiblePackages = [
      packageName,
      `@alanos/${packageName.replace('plugin-', '')}`,
      `@alanos/plugin-${packageName.replace('plugin-', '')}`,
    ];
