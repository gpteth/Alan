...
export const findPluginPackageName = (
  ...
  // Potential package names to check (prioritize @alanos/ over @alanos/)
  const possibleNames = [
    pluginInput, // Check the raw input first
    `@alanos/plugin-${normalizedBase}`, // Prioritize @alanos/ scope
    `@alanos/${normalizedBase}`, // Might be needed if input was 'plugin-abc' -> base 'abc' -> check '@alanos/abc'
    `@alanos/plugin-${normalizedBase}`, // Check alternative scope
    `plugin-${normalizedBase}`,
    `@alanos/${normalizedBase}`,
  ];
...
export const extractPackageName = (pluginInput: string): string => {
  ...
  if (githubUrlMatch) {
    ...
    // e.g., github:alanos-plugins/plugin-discord -> @alanos/plugin-discord
    return `@${owner}/${repo}`;
  }
