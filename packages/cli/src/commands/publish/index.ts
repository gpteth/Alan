...
export const publish = new Command()
  ...
  .action(async (opts: PublishOptions) => {
    try {
      ...
      if (directoryInfo.type === 'alanos-plugin') {
        detectedType = 'plugin';
        console.info('Detected alanOS plugin using standardized directory detection');
      } else if (directoryInfo.type === 'alanos-project') {
        detectedType = 'project';
        console.info('Detected alanOS project using standardized directory detection');
      } else {
        ...
        } else if (packageJson.alan?.type === 'project') {
          detectedType = 'project';
          console.info('Detected project from package.json alan.type (legacy format)');
      ...
      const placeholderReplacements: Record<string, PlaceholderReplacement> = {
        ...
        'alanos-plugin-starter': {
          check: () => packageJson.name === '@alanos/plugin-starter',
          replace: () => {
        ...
        '${PLUGINDESCRIPTION}': {
          ...
          replace: () => {
            ...
            packageJson.description = `alanOS plugin for ${simpleName}`;
      ...
      console.log('   Use standard npm and git workflows, not the alanOS CLI:');
      ...
      console.log('\n   The alanOS registry will automatically sync with npm updates.');
      console.log('   Only use "alanos publish" for initial publishing of new plugins.');
    } catch (error) {
      handleError(error);
    }
  });
