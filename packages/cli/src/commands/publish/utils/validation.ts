...
export async function validatePluginRequirements(
  ...
  const expectedDefaultDesc = `alanOS plugin for ${pluginDirName.replace('plugin-', '')}`;
  if (
...
export function displayRegistryPublicationMessage(
  ...
  if (opts.npm) {
    ...
    console.info(`1. Fork the registry repository at https://github.com/alanos/registry`);
