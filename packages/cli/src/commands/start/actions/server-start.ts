...
import { logger, type Character, type ProjectAgent } from '@alanos/core';
 ...
 *
 * Initializes the database, creates the server instance, configures port settings, and starts the specified agents or default alan character.
 */
export async function startAgents(options: ServerStartOptions): Promise<void> {
  ...
  // In production/global: node_modules/@alanos/cli/dist/commands/start/actions -> node_modules/@alanos/cli/dist
    ...
    while (currentDir !== path.dirname(currentDir)) {
      ...
      if (existsSync(packageJsonPath)) {
        try {
          ...
          if (packageJson.name === '@alanos/cli') {
  ...
  // Default fallback to alan character
  else {
    const alanCharacter = getalanCharacter();
    await startAgent(alanCharacter, server);
  }
}
