...
export const start = new Command()
  .name('start')
  .description('Build and start the alan agent server')
  ...
  .action(async (options: StartOptions & { character?: string[] }) => {
    try {
      ...
      const isMonorepo = dirInfo.type === 'alanos-monorepo';

      if (!isMonorepo && !process.env.alan_TEST_MODE) {
        try {
      ...
      if (options.character && options.character.length > 0) {
        // Load @alanos/server module for character loading
        ...
        try {
          ...
          // except those explicitly detected as non-alanOS (covers projects, plugins, monorepos, etc.)
          if (dirInfo.hasPackageJson && dirInfo.type !== 'non-alanos-dir') {
