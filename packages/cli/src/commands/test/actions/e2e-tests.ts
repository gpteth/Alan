...
import { logger, type IAgentRuntime, type ProjectAgent, Project } from '@alanos/core';
...
export async function runE2eTests(
  ...
  if (!options.skipBuild) {
    try {
      ...
      const isPlugin = projectInfo.type === 'alanos-plugin';
  ...
  try {
    ...
    // Load @alanos/server from the project's node_modules
    const moduleLoader = getModuleLoader();
    const serverModule = await moduleLoader.load('@alanos/server');
    const { AgentServer, jsonToCharacter, loadCharacterTryPath } = serverModule;

    // Set up standard paths and load .env
    const alanDir = path.join(process.cwd(), '.alan');
    // Create unique database directory for each test run to avoid conflicts
    const packageName = path.basename(process.cwd());
    const timestamp = Date.now();
    const uniqueDbDir = path.join(process.cwd(), '.alandb-test', `${packageName}-${timestamp}`);
    const alanDbDir = uniqueDbDir;
    const envInfo = await UserEnvironment.getInstanceInfo();
    const envFilePath = envInfo.paths.envFilePath;

    console.info('Setting up environment...');
    console.info(`alan directory: ${alanDir}`);
    console.info(`Database directory: ${alanDbDir}`);
    ...
    if (fs.existsSync(alanDbDir)) {
      console.info(`Cleaning up existing database directory: ${alanDbDir}`);
      try {
        fs.rmSync(alanDbDir, { recursive: true, force: true });
    ...
    console.info(`Creating fresh database directory: ${alanDbDir}`);
    fs.mkdirSync(alanDbDir, { recursive: true });
    console.info(`Created database directory: ${alanDbDir}`);

    // Set the database directory in environment variables to ensure it's used
    process.env.PGLITE_DATA_DIR = alanDbDir;
    console.info(`Set PGLITE_DATA_DIR to: ${alanDbDir}`);
    ...
    try {
      ...
      try {
        ...
        // When testing a plugin, import and use the default alan character
        ...
        if (project.isPlugin || project.agents.length === 0) {
          ...
          try {
            ...
            projectAgents.push({
              character: defaultalanCharacter,
        ...
        for (let i = 0; i < runtimes.length; i++) {
          ...
          const results = await testRunner.runTests({
            ...
            skipPlugins: currentDirInfo.type !== 'alanos-plugin',
            // Only run project tests if we're actually in a project directory
            skipProjectTests: currentDirInfo.type !== 'alanos-project',
        ...
        // Clean up the alan_TESTING_PLUGIN environment variable
        if (process.env.alan_TESTING_PLUGIN) {
          delete process.env.alan_TESTING_PLUGIN;
        }

        // Clean up database directory after tests complete
        try {
          if (fs.existsSync(alanDbDir)) {
            console.info(`Cleaning up test database directory: ${alanDbDir}`);
            fs.rmSync(alanDbDir, { recursive: true, force: true });
            console.info(`Successfully cleaned up test database directory`);
          }
          // Also clean up the parent test directory if it's empty
          const testDir = path.dirname(alanDbDir);
