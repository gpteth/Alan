...
export async function performCliUpdate(options: GlobalUpdateOptions = {}): Promise<boolean> {
  try {
    ...
    if (targetVersion === 'latest') {
      const fetchedVersion = await fetchLatestVersion('@alanos/cli');
      if (!fetchedVersion) {
    ...
    if (!options.skipBunMigration) {
      ...
      if (npmInstallation) {
        ...
        try {
          ...
          try {
            await bunExecInherit('npm', ['install', '-g', `@alanos/cli@${latestVersion}`]);
            console.log(`CLI updated successfully to version ${latestVersion} [✓]`);
            return true;
          } catch (npmError) {
            throw new Error(
    ...
    try {
      await bunExecInherit('bun', ['add', '-g', `@alanos/cli@${latestVersion}`]);
      console.log(`CLI updated successfully to version ${latestVersion} [✓]`);
      return true;
    } catch (bunError) {
