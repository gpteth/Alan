 ...
 *
 * Updates alanOS dependencies in a project or plugin, with support for dry-run mode, major version confirmation, and optional build step.
 */
export async function updateDependencies(
  ...
  if (!hasUpdates) {
    console.log('All alanOS packages are up to date! [✓]');
    return;
  }
  ...
  if (dryRun) {
    console.log('\nTo apply these updates, run: alanos update');
    return;
  }
