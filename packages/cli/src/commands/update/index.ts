...
export const update = new Command()
  .name('update')
  .description('Update alanOS CLI and project dependencies')
    ...
    try {
      ...
      if (updateCli) {
        ...
        if (isNpx || isBunx) {
          ...
          console.info(' bun install -g @alanos/cli');
      ...
      if (updatePackages) {
        ...
        if (!directoryInfo) {
          ...
          console.info('To create a new alanOS project, use: alanos create <project-name>');
          return;
        }
        ...
        const isPlugin = directoryInfo.type === 'alanos-plugin';

        if (directoryInfo.alanPackageCount === 0) {
          console.info('No alanOS packages found in this project.');
          console.info(
            "This might be a new project that hasn't installed alanOS dependencies yet."
          );
          console.info('Consider adding alanOS packages first, such as: bun add @alanos/core');
          return;
        }

        console.info(
          `Found ${directoryInfo.alanPackageCount} alanOS package(s) to check for updates`
        );
        ...
        if (options.check) {
          ...
          console.log(`${projectType} successfully updated to the latest alanOS packages`);
        }
      }
    } catch (error) {
      handleError(error);
