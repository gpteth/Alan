...
export async function loadProject(dir: string): Promise<Project> {
  try {
    ...
    if (!main) {
      ...
      // Create a fallback project with the default alan character
      // Use deterministic UUID based on character name to match runtime behavior
      const defaultCharacterName = 'alan (Default)';
      const alanCharacter = getalanCharacter(); // Get the filtered character based on env vars
      const defaultAgent: ProjectAgent = {
        ...
        init: async () => {
          logger.info('Initializing default alan character');
        },
      };
    ...
    if (moduleIsPlugin) {
      ...
      try {
        ...
        // Use the alan character as our test agent
        // Use deterministic UUID based on character name to match runtime behavior
        const characterName = 'alan (Test Mode)';
        const alanCharacter = getalanCharacter(); // Get the filtered character based on env vars
        const testCharacter: Character = {
          ...alanCharacter,
          id: stringToUuid(characterName) as UUID,
          name: characterName,
          system: `${alanCharacter.system} Testing the plugin: ${completePlugin.name}.`,
        };

        logger.info(`Using alan character as test agent for plugin: ${completePlugin.name}`);
        ...
        const testAgent: ProjectAgent = {
          ...
          init: async () => {
            logger.info(`Initializing alan test agent for plugin: ${completePlugin.name}`);
            // The plugin will be registered automatically in runtime.initialize()
          },
        };

        // Since we're in test mode, alan (our test agent) needs to already exist in the database
