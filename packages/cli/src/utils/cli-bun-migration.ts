...
export async function isCliInstalledViaNpm(): Promise<boolean> {
  try {
    const { stdout } = await bunExecSimple('npm', ['list', '-g', '@alanos/cli', '--depth=0']);
    return stdout.includes('@alanos/cli');
  } catch (error) {
    // Also check if the current alanos command points to npm installation
    try {
      const { stdout: whichOutput } = await bunExecSimple('which', ['alanos']);
...
async function removeNpmInstallation(): Promise<void> {
  logger.info('Removing npm installation of @alanos/cli...');
  await bunExecInherit('npm', ['uninstall', '-g', '@alanos/cli']);
}
...
async function installCliWithBun(version: string): Promise<void> {
  ...
  await bunExecInherit('bun', ['add', '-g', `@alanos/cli@${version}`]);
}
...
async function verifyCliInstallation(expectedVersion: string): Promise<boolean> {
  try {
    const { stdout } = await bunExecSimple('alanos', ['-v']);
...
export async function migrateCliToBun(targetVersion: string): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      await bunExec('bun', ['remove', '-g', '@alanos/cli'], { stdio: 'ignore' });
    } catch {
