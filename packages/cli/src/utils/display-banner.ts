...
export async function getLatestCliVersion(currentVersion: string): Promise<string | null> {
  ...
  try {
    ...
    const { stdout } = await bunExecSimple('npm', ['view', '@alanos/cli', 'time', '--json']);
...
export function showUpdateNotification(currentVersion: string, latestVersion: string) {
  ...
  console.log(
    `${blue}│${orange} Run ${green}${bold}bun i -g @alanos/cli@latest${reset}${orange} to get the latest features${' '.repeat(width - 2 - ` Run bun i -g @alanos/cli@latest to get the latest features`.length)}${blue}│${reset}`
  );
  console.log(border);
  console.log('');
}
