 ...
 */
const SAMPLE_ENV_TEMPLATE = `### alanOS Environment Variables ###
...
## MODEL PROVIDER KEYS ##
## alan is compatible with a wide array of model providers. Many have OpenAI compatible APIs,
...
### DATABASE ###
# By default, alan will use a local pglite instance
...
/**
 * Retrieves the standard directory paths used by alan for configuration and database storage.
 *
 * @returns An object containing the alan configuration directory, the alan database directory for the current project, and the path to the alan `.env` file.
 */
export async function getalanDirectories(targetProjectDir?: string) {
  const userEnv = UserEnvironment.getInstance();
  const paths = await userEnv.getPathInfo();

  const projectRoot = targetProjectDir || paths.monorepoRoot || process.cwd();
  const alanDir = targetProjectDir ? path.resolve(targetProjectDir, '.alan') : paths.alanDir;
  const envFilePath = targetProjectDir ? path.resolve(targetProjectDir, '.env') : paths.envFilePath;

  logger.debug('alan directories:', {
    alanDir,
    projectRoot,
    targetProjectDir: targetProjectDir || 'none',
  });

  const defaultalanDbDir = path.resolve(projectRoot, '.alan', '.alandb');
  // Pass targetProjectDir to resolvePgliteDir to ensure it uses the correct base directory
  const alanDbDir = await resolvePgliteDir(undefined, defaultalanDbDir, targetProjectDir);

  return { alanDir, alanDbDir, envFilePath };
}
...
/**
 * Ensures the alan configuration directory exists and returns standard alan directory paths.
 *
 * @returns An object containing paths for the alan configuration directory, the alan database directory, and the `.env` file.
 */
export async function ensurealanDir(targetProjectDir?: string) {
  const dirs = await getalanDirectories(targetProjectDir);
  await ensureDir(dirs.alanDir);

  // Also create registry-cache.json and config.json files if they don't exist
  const registryCachePath = path.join(dirs.alanDir, 'registry-cache.json');
  const configPath = path.join(dirs.alanDir, 'config.json');
 ...
 * @param alanDbDir The directory for PGLite database
 * @param envFilePath Path to the .env file
 */
export async function setupPgLite(
  ...
  const dirs = await ensurealanDir(targetProjectDir);
  const { alanDbDir, envFilePath } = dirs;

  // Use provided parameters or defaults from dirs
  const targetDbDir = dbDir || alanDbDir;
...
export async function configureDatabaseSettings(reconfigure = false): Promise<string | null> {
  ...
  const { alanDbDir, envFilePath } = await ensurealanDir();
  ...
  const pgliteDataDir = await resolvePgliteDir(undefined, alanDbDir);
  ...
  await setupPgLite(alanDbDir, envFilePath);
  return null;
}
