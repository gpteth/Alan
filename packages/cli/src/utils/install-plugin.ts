...
function getCliDirectory(): string | null {
  try {
    ...
    if (cliPath.includes('node_modules/@alanos/cli')) {
      // Go up to the CLI package root
      const cliDir = path.dirname(
        cliPath.split('node_modules/@alanos/cli')[0] + 'node_modules/@alanos/cli'
      );
...
async function attemptInstallation(
  ...
  try {
    ...
    if (skipVerification || process.env.alan_SKIP_PLUGIN_VERIFY) {
      logger.info(
  ...
  if (info.npm?.repo) {
    ...
    if (result.success) {
      ...
      if (
        ...
        !skipVerification &&
        !process.env.alan_SKIP_PLUGIN_VERIFY
      ) {
