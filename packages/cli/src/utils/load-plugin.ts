...
const importStrategies: ImportStrategy[] = [
  ...
  // Try workspace dependencies (for monorepo packages)
  {
    ...
    tryImport: async (repository: string) => {
      if (repository.startsWith('@alanos/plugin-')) {
        // Try to find the plugin in the workspace
        const pluginName = repository.replace('@alanos/', '');
...
/**
 * Determines if a package name is from the alanOS ecosystem
 */
function isalanOSPackageName(repository: string): boolean {
  return repository.startsWith('@alanos/') || repository.startsWith('@alanos-plugins/');
}
...
function getStrategiesForPlugin(repository: string): ImportStrategy[] {
  const isalanOS = isalanOSPackageName(repository);

  if (isalanOS) {
    // alanOS ecosystem plugins: try all strategies
 ...
 * alanOS ecosystem plugins (@alanos/*) use all strategies,
...
export async function loadPluginModule(repository: string): Promise<any | null> {
  ...
  logger.debug(
    `Loading ${isalanOS ? 'alanOS' : 'third-party'} plugin: ${repository} (${strategies.length} strategies)`
  );
