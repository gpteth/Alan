 ...
 *
 * This utility checks for a local @alanos/cli installation and delegates to it
...
async function delegateToLocalCli(localCliPath: string): Promise<void> {
  return new Promise((resolve, reject) => {
    logger.info('Using local @alanos/cli installation');
...
export async function tryDelegateToLocalCli(): Promise<boolean> {
  try {
    ...
    if (process.env._alan_CLI_DELEGATION_DEPTH) {
      const depth = parseInt(process.env._alan_CLI_DELEGATION_DEPTH, 10);
      if (depth > 0) {
    ...
    process.env._alan_CLI_DELEGATION_DEPTH = '1';
