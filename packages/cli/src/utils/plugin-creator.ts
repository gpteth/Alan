...
export class PluginCreator {
  ...
  async create(pluginSpec?: PluginSpecification): Promise<CreationResult> {
    ...
    try {
      ...
      logger.info(`4. Add to your alanOS project\n`);
  ...
  private async createFromTemplate(pluginName: string): Promise<void> {
    ...
    // Use alanos create command to create from template
    try {
      await bunExec(
        'bunx',
        ['@alanos/cli', 'create', `plugin-${pluginName}`, '-t', 'plugin-starter'],
        {
          cwd: tempDir,
          stdio: 'pipe',
        }
      ...
      // Fallback to manual creation if alanos create fails
      logger.warn('Failed to use alanos create, creating structure manually');
  ...
  private async createPluginStructureManually(pluginName: string): Promise<void> {
    ...
    // Create package.json
    const packageJson = {
      name: `@alanos/plugin-${pluginName}`,
      version: '0.1.0',
      description: `alanOS ${pluginName} plugin`,
      main: 'dist/index.js',
...
`;
    ...
    // Create README.md
    const readme = `# @alanos/plugin-${pluginName}

alanOS ${pluginName} plugin

## Installation

\`\`\`bash
bun install @alanos/plugin-${pluginName}
\`\`\`
...
import { plugin${pluginName
      ...
      .join('')} } from '@alanos/plugin-${pluginName}';

// Add to your alanOS configuration
\`\`\`
`;
    ...
    const indexContent = `import { Plugin } from '@alanos/core';
...
`;
  ...
  private async generateDetailedSpecification(spec: PluginSpecification): Promise<string> {
    const prompt = `You are creating a detailed technical specification for an alanOS plugin.
...
- ALL imports must come from @alanos/core ONLY
- NO imports from @alanos/plugin, @alanos/types, @alanos/logger (these don't exist)
- Use: import { Plugin, Action, AgentRuntime, logger, Memory, State } from '@alanos/core'
...
4. **Integration Points**: How the plugin integrates with alanOS runtime
...
import { PgliteDatabaseAdapter } from '@alanos/plugin-sql';
\`\`\`
...
import { logger } from '@alanos/logger';
import { Action } from '@alanos/types';
import { PgliteDatabaseAdapter } from '@alanos/plugin-sql';
\`\`\`
...
You are now going to implement this plugin following alanOS 1.0.0 best practices:

### 1. Core Implementation
- **Use TypeScript** for all code
- **Follow the alanOS plugin structure** exactly
- **Implement all components** specified above
- **Create comprehensive tests** for each component
- **Use proper error handling** throughout
- **Add detailed logging** using the alanOS logger
...
### 3. Import Compliance
- **ALL imports from @alanos/core ONLY**
...
- ✅ All imports come from @alanos/core only
- ✅ No direct database adapter imports
...
Make all necessary changes to fix the issues and ensure the plugin builds and all tests pass.`;
  ...
  private async validateProductionReadiness(): Promise<ValidationResult> {
    ...
    const prompt = `You are reviewing a newly generated alanOS plugin for production readiness.
...
- ✅ ALL imports must come from @alanos/core ONLY
- ❌ NO imports from @alanos/plugin, @alanos/types, @alanos/logger, etc. (these don't exist)
- ❌ NO direct database adapter imports (PgliteDatabaseAdapter, PgDatabaseAdapter)
- ✅ Correct import format: import { Plugin, Action, AgentRuntime, logger } from '@alanos/core'
...
### 5. Code Quality
- ✅ Proper error handling throughout
- ✅ Follows alanOS 1.0.0 patterns and best practices
...
1. Import compliance is MANDATORY - any non-@alanos/core imports = NOT production ready
