...
/**
 * Publishes a package to GitHub and optionally updates the alanOS registry for plugins.
 *
 * For both plugins and projects, this function creates or verifies a GitHub repository, pushes the local code, and returns success. For plugins (unless {@link skipRegistry} is true), it also updates the alanOS registry by forking the registry repository, creating a branch, updating or creating the package metadata, updating the registry index, and opening a pull request.
 *
...
export async function publishToGitHub(
  ...
  if (!isTest) {
    ...
    const description = packageJson.description || `alanOS ${packageJson.packageType}`;

    // Set the appropriate topic based on package type - only one topic, no mixing
    let topic;
    if (packageJson.packageType === 'plugin') {
      topic = 'alanos-plugins';
    } else if (packageJson.packageType === 'project') {
      topic = 'alanos-projects';
    } else {
      topic = 'alanos-plugins'; // Default to plugins if type is unknown
    }
  ...
  // Use the actual npm package name from package.json (not @alanos/ prefix)
