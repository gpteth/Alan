...
const alan_DIR = path.join(process.cwd(), '.alan');
const REGISTRY_SETTINGS_FILE = path.join(alan_DIR, 'registrysettings.json');
// Use resolveEnvFile to match how credentials are saved, with fallback to ~/.alan/.env
const ENV_FILE = resolveEnvFile() || path.join(alan_DIR, '.env');
const REGISTRY_CACHE_FILE = path.join(alan_DIR, 'registry-cache.json');
...
export async function ensurealanDir() {
  try {
    await fs.mkdir(alan_DIR, { recursive: true });
  } catch (error) {
...
export async function getGitHubToken(): Promise<string | undefined> {
  try {
    ...
    const globalEnvPath = path.join(alan_DIR, '.env');
...
export async function setGitHubToken(token: string) {
  ...
  try {
    ...
    try {
      ...
      envContent = '# alan environment variables\n\n';
    }
...
function normalizePackageName(packageName: string): string {
  ...
  } else if (packageName.startsWith('@alanos/')) {
    return packageName.replace(/^@alanos\//, '');
  }
  return packageName;
}
...
const DEFAULT_REGISTRY: Record<string, string> = {
  '@alanos/plugin-anthropic': 'github:alanos-plugins/plugin-anthropic',
  '@alanos/plugin-bootstrap': 'github:alanos-plugins/plugin-bootstrap',
  '@alanos/plugin-browser': 'github:alanos-plugins/plugin-browser',
  '@alanos/plugin-discord': 'github:alanos-plugins/plugin-discord',
  '@alanos/plugin-elevenlabs': 'github:alanos-plugins/plugin-elevenlabs',
  '@alanos/plugin-evm': 'github:alanos-plugins/plugin-evm',
  '@alanos/plugin-farcaster': 'github:alanos-plugins/plugin-farcaster',
  '@alanos/plugin-groq': 'github:alanos-plugins/plugin-groq',
  '@alanos/plugin-mcp': 'github:alanos-plugins/plugin-mcp',
  '@alanos/plugin-messari-ai-toolkit': 'github:messari/plugin-messari-ai-toolkit',
  '@alanos/plugin-morpheus': 'github:bowtiedbluefin/plugin-morpheus',
  '@alanos/plugin-node': 'github:alanos-plugins/plugin-node',
  '@alanos/plugin-ollama': 'github:alanos-plugins/plugin-ollama',
  '@alanos/plugin-openai': 'github:alanos-plugins/plugin-openai',
  '@alanos/plugin-pdf': 'github:alanos-plugins/plugin-pdf',
  '@alanos/plugin-redpill': 'github:alanos-plugins/plugin-redpill',
  '@alanos/plugin-solana': 'github:alanos-plugins/plugin-solana',
  '@alanos/plugin-sql': 'github:alanos-plugins/plugin-sql',
  '@alanos/plugin-storage-s3': 'github:alanos-plugins/plugin-storage-s3',
  '@alanos/plugin-tee': 'github:alanos-plugins/plugin-tee',
  '@alanos/plugin-telegram': 'github:alanos-plugins/plugin-telegram',
  '@alanos/plugin-twitter': 'github:alanos-plugins/plugin-twitter',
  '@alanos/plugin-venice': 'github:alanos-plugins/plugin-venice',
  '@alanos/plugin-video-understanding': 'github:alanos-plugins/plugin-video-understanding',
};
...
export function normalizePluginName(pluginName: string): string[] {
  ...
  if (pluginName.includes('/')) {
    // Handle formats like "alanos/plugin-ton" or "alanos-plugins/plugin-ton"
    const parts = pluginName.split('/');
    baseName = parts[parts.length - 1];
  } else if (pluginName.startsWith('@')) {
    // Handle scoped package format like "@alanos/plugin-ton"
    const parts = pluginName.split('/');
    if (parts.length > 1) {
      baseName = parts[1];
    }
  ...
  return [
    ...
    `@alanos/${baseName}`, // Scoped with alanos
    `@alanos/plugin-${baseName}`, // Scoped with alanos and plugin prefix
  ];
}
...
export async function validateDataDir(): Promise<boolean> {
  ...
  if (!status.exists) {
    logger.warn('alanOS data directory not found. Initializing...');
