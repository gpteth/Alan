 ...
 * 4. `./.alan/.alandb` relative to the current working directory.
 *
...
export async function resolvePgliteDir(
  ...
  // The fallbackDir passed from getalanDirectories will be monorepoRoot + '.alandb' or similar.
  // If fallbackDir is not provided (e.g. direct call to resolvePgliteDir),
  // then we construct the default path using projectRoot.
  const defaultBaseDir = path.join(projectRoot, '.alan', '.alandb');

  const base = dir ?? process.env.PGLITE_DATA_DIR ?? fallbackDir ?? defaultBaseDir;

  // Resolve and migrate legacy default (<projectRoot>/.alandb) if detected
  const resolved = expandTildePath(base, projectRoot);
  const legacyPath = path.join(projectRoot, '.alandb');
  if (resolved === legacyPath) {
    const newPath = path.join(projectRoot, '.alan', '.alandb');
