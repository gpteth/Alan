# alanOS Plugin Upgrade System

This directory contains the automated plugin upgrade system for migrating alanOS plugins from version 0.x to 1.x.
...
```bash
alanos plugins upgrade <path> [options]
```
...
alanos plugins upgrade https://github.com/user/plugin-name

# Upgrade a local plugin with API key
alanos plugins upgrade ./my-plugin --api-key sk-ant-...
...
- Runs `alanos test` to validate the migration
...
- `CLAUDE.md`: Base migration instructions for alanOS 0.x to 1.x
...
```bash
alanos plugins upgrade ./plugin --skip-tests
```
...
- ✅ Test command fallback chain (alanos → bun → npm)
