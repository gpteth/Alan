...
export class MigrationGuideLoader {
  ...
  private getEmbeddedMigrationGuide(): string {
    return `# alanOS Plugin Migration Guide - Essential Steps
...
For complete details, refer to the comprehensive guides in the alanOS documentation.`;
  }

  private getEmbeddedTestingGuide(): string {
    return `# alanOS Testing Guide - Essential Requirements
...
Run: bun test --coverage`;
  ...
  private getEmbeddedCompletionGuide(): string {
    return `# alanOS Completion Requirements
...
- Version management`;
  ...
  getRelevantGuidesForIssue(issue: string): GuideSearchResult[] {
    const issueMap: Record<string, string[]> = {
      import_error: ['import', 'path', 'alanLogger', 'logger'],
  ...
  generateMigrationContext(): string {
    const context = [
      '# alanOS PLUGIN MIGRATION GUIDE REFERENCES',
      '',
  ...
  getAllGuidesContent(): string {
    const content = [
      '# COMPLETE alanOS MIGRATION KNOWLEDGE BASE',
      '',
