...
export class UserEnvironment {
  ...
  private async getCLIInfo(): Promise<CLIInfo> {
    ...
    try {
      ...
      return {
        ...
        name: packageJson.name || '@alanos/cli',
        path: process.argv[1] || '',
      };
    } catch (error) {
      logger.warn(
  ...
  /**
   * Detects the active package manager - always returns bun for alanOS CLI
   */
  private async getPackageManagerInfo(): Promise<PackageManagerInfo> {
    logger.debug('[UserEnvironment] Using bun as the package manager for alanOS CLI');
    ...
    if (typeof Bun !== 'undefined' && Bun.version) {
      ...
      try {
        ...
        if (!version) {
          ...
          logger.error(
            `${emoji.error('Bun is required for alanOS CLI but is not installed or not found in PATH.')}`
          );
          logger.error('');
          logger.error(
          ...
          // Force exit the process - Bun is required for alanOS CLI
      ...
      if (existsSync(corePackagePath)) {
        try {
          ...
          if (stats.isDirectory()) {
            // Additional validation: check if this looks like the alanOS monorepo
            const packageJsonPath = path.join(currentDir, 'package.json');
            if (existsSync(packageJsonPath)) {
              const packageJsonContent = readFileSync(packageJsonPath, 'utf8');
              const packageJson = JSON.parse(packageJsonContent);
              // Verify this is actually the alanOS monorepo
              if (packageJson.name?.includes('alan') || packageJson.workspaces) {
                return currentDir;
              }
            }
          }
  ...
  public async getPathInfo(): Promise<PathInfo> {
    ...
    const alanDir = path.join(projectRootForPaths, '.alan');
    ...
    return {
      alanDir,
      envFilePath,
      configPath: path.join(alanDir, 'config.json'),
      pluginsDir: path.join(alanDir, 'plugins'),
      monorepoRoot,
