...
@test "context: CLI shows help from dist" {
  ...
  assert_cli_success
  assert_output --partial "Usage: alanos"
...
@test "context: CLI commands work from monorepo root" {
  ...
  assert_cli_success
  assert_output --partial "Create a new alanOS project"
}

@test "context: Environment variables are respected" {
  export alan_NO_AUTO_INSTALL=true
  export NO_COLOR=1
  export alan_LOG_LEVEL=debug
