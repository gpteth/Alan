...
describe('alanOS Agent Commands', () => {
  ...
  beforeAll(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'alan-test-agent-'));
    ...
    // Start the alanOS server with a default character
    console.log(`[DEBUG] Starting alanOS server on port ${testServerPort}`);
    ...
    // Verify alanos command is available
    try {
      bunExecSync('alanos --version', { encoding: 'utf8', timeout: 5000 });
    } catch (error) {
      console.error(
        '[ERROR] alanos command not available. Run "bun link" in the CLI package first.'
      );
      throw new Error('alanos command not available');
    }

    // Spawn server process using alanos command
    console.log(`[DEBUG] Spawning server with: alanos start`);

    try {
      const proc = Bun.spawn(
        ['alanos', 'start', '--port', testServerPort, '--character', defaultCharacter],
        {
          env: {
            ...process.env,
            LOG_LEVEL: 'debug',
            PGLITE_DATA_DIR: `${testTmpDir}/alandb`,
      ...
      console.error(`[ERROR] Command: alanos start`);
    ...
    for (const character of ['max', 'shaw']) {
      ...
      try {
        ...
        bunExecSync(
          `alanos agent start --remote-url ${testServerUrl} --path "${characterPath}"`,
          platformOptions
        );
  ...
  it('agent help displays usage information', async () => {
    const result = bunExecSync(`alanos agent --help`, getPlatformOptions({ encoding: 'utf8' }));
    expect(result).toContain('Usage: alanos agent');
  });

  it('agent list returns agents', async () => {
    const result = bunExecSync(
      `alanos agent list --remote-url ${testServerUrl}`,
      getPlatformOptions({
        encoding: 'utf8',
      })
    );
  ...
  it('agent list works with JSON flag', async () => {
    const result = bunExecSync(
      `alanos agent list --remote-url ${testServerUrl} --json`,
      getPlatformOptions({
        encoding: 'utf8',
      })
    );
  ...
  it('agent get shows details with name parameter', async () => {
    const result = bunExecSync(
      `alanos agent get --remote-url ${testServerUrl} -n Ada`,
      getPlatformOptions({
        encoding: 'utf8',
      })
    );
  ...
  it('agent get with JSON flag shows character definition', async () => {
    const result = bunExecSync(
      `alanos agent get --remote-url ${testServerUrl} -n Ada --json`,
      getPlatformOptions({
        encoding: 'utf8',
      })
    );
  ...
  it('agent get with output flag saves to file', async () => {
    ...
    bunExecSync(
      `alanos agent get --remote-url ${testServerUrl} -n Ada --output "${outputFile}"`,
  ...
  it('agent start loads character from file', async () => {
    ...
    try {
      const result = bunExecSync(
        `alanos agent start --remote-url ${testServerUrl} --path "${maxPath}"`,
        getPlatformOptions({ encoding: 'utf8' })
      );
  ...
  it('agent start works with name parameter', async () => {
    try {
      bunExecSync(
        `alanos agent start --remote-url ${testServerUrl} -n Ada`,
        getPlatformOptions({
          encoding: 'utf8',
        })
      );
  ...
  it('agent start handles non-existent agent fails', async () => {
    ...
    try {
      bunExecSync(
        `alanos agent start --remote-url ${testServerUrl} -n ${nonExistentName}`,
        getPlatformOptions({
          encoding: 'utf8',
          stdio: 'pipe',
        })
  ...
  it('agent stop works after start', async () => {
    ...
    try {
      const result = bunExecSync(
        `alanos agent stop --remote-url ${testServerUrl} -n Ada`,
        getPlatformOptions({
          encoding: 'utf8',
        })
      );
  ...
  it('agent set updates configuration correctly', async () => {
    ...
    const result = bunExecSync(
      `alanos agent set --remote-url ${testServerUrl} -n Ada -f "${configFile}"`,
      getPlatformOptions({ encoding: 'utf8' })
    );
  ...
  it('agent stop --all works for stopping all agents', async () => {
    ...
    try {
      ...
      expect(result).toMatch(/(All alanOS agents stopped|stopped successfully)/);
    } catch (e: any) {
      // The command may succeed even if no agents are running
      // Handle case where stdout/stderr might be undefined
      const output = (e.stdout || '') + (e.stderr || '') + (e.message || '');
      expect(output).toMatch(
        /(stopped|All alanOS agents stopped|Windows|WSL|requires Unix-like commands)/
      );
    }
  });
});
