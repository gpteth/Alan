...
describe('alanOS Create Commands', () => {
  ...
  beforeEach(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'alan-test-'));
  ...
  it('create --help shows usage', () => {
    ...
    expect(result).toContain('Usage: alanos create');
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync(
        'alanos create my-default-app --yes',
        getPlatformOptions({
          encoding: 'utf8',
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('alanos create my-plugin-app --yes --type plugin', {
        encoding: 'utf8',
        timeout: TEST_TIMEOUTS.PROJECT_CREATION,
      }) as string;
  ...
  it('create agent succeeds', async () => {
    ...
    const result = bunExecSync('alanos create my-test-agent --yes --type agent', {
      encoding: 'utf8',
    }) as string;
  ...
  it('rejects creating project in existing directory', async () => {
    ...
    try {
      const output = bunExecSync('alanos create existing-app --yes', {
        encoding: 'utf8',
      }) as string;
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('alanos create . --yes', {
        encoding: 'utf8',
        timeout: TEST_TIMEOUTS.PROJECT_CREATION,
      }) as string;
  ...
  it('rejects invalid project name', () => {
    let result: { status: number; output: string };
    try {
      const output = bunExecSync('alanos create Invalid-Name! --yes', {
        encoding: 'utf8',
      }) as string;
      throw new Error(`Command should have failed but succeeded with output: ${output}`);
    } catch (e: any) {
  ...
  it('rejects invalid project type', () => {
    let result: { status: number; output: string };
    try {
      const output = bunExecSync('alanos create bad-type-proj --yes --type bad-type', {
        encoding: 'utf8',
      }) as string;
      throw new Error(`Command should have failed but succeeded with output: ${output}`);
    } catch (e: any) {
  ...
  // create-alan parity tests
  it('create-alan default project succeeds', async () => {
    // Use cross-platform directory removal
    await crossPlatform.removeDir('my-create-app');

    // Skip this test - create-alan command not available
    console.warn('Skipping create-alan test - command not available');
  }, 60000);

  it('create-alan plugin project succeeds', async () => {
    // Use cross-platform directory removal
    await crossPlatform.removeDir('plugin-my-create-plugin');

    // Skip this test - create-alan command not available
    console.warn('Skipping create-alan plugin test - command not available');
  }, 60000);

  it('create-alan agent succeeds', async () => {
    // Use cross-platform file removal
    await crossPlatform.removeFile('my-create-agent.json');

    // Skip this test - create-alan command not available
    console.warn('Skipping create-alan agent test - command not available');
  }, 60000);
  ...
  describe('CLAUDE.md File Creation', () => {
    it(
      ...
      async () => {
        ...
        const result = bunExecSync(
          'alanos create claude-md-test-project --yes',
          getPlatformOptions({
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          })
        ...
        expect(claudeMdContent).toContain('alanOS Agent Project Development Guide for Claude');
        expect(claudeMdContent).toContain('Project Type** | alanOS Agent Project');
        ...
        expect(claudeMdContent).toContain('alanos dev');
        expect(claudeMdContent).toContain('alanos start');
      },
      TEST_TIMEOUTS.INDIVIDUAL_TEST
    );
    ...
    it(
      ...
      async () => {
        ...
        const result = bunExecSync(
          'alanos create claude-md-test --yes --type plugin',
          getPlatformOptions({
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          })
        ...
        expect(claudeMdContent).toContain('alanOS Plugin Development Guide for Claude');
        expect(claudeMdContent).toContain('Project Type** | alanOS Plugin');
    ...
    it(
      ...
      async () => {
        ...
        const createProcess = Bun.spawn(
          ['alanos', 'create', pluginName, '--type', 'plugin', '--yes'],
          {
            stdout: 'ignore',
            stderr: 'ignore',
            stdin: 'ignore',
  ...
  describe('--dir Flag Removal (Breaking Change)', () => {
    it('rejects --dir flag with helpful error message', () => {
      let result: { status: number; output: string };
      try {
        const output = bunExecSync('alanos create my-project --dir /some/path', {
          encoding: 'utf8',
        }) as string;
        throw new Error(`Command should have failed but succeeded with output: ${output}`);
      } catch (e: unknown) {
    ...
    it('rejects -d shorthand flag', () => {
      let result: { status: number; output: string };
      try {
        const output = bunExecSync('alanos create my-project -d /some/path', {
          encoding: 'utf8',
        }) as string;
        throw new Error(`Command should have failed but succeeded with output: ${output}`);
      } catch (e: unknown) {
    ...
    it(
      ...
      async () => {
        ...
        try {
          const result = bunExecSync('alanos create my-current-dir-project --yes', {
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          }) as string;
    ...
    it(
      ...
      async () => {
        ...
        // Before: alanos create my-project --dir /path/to/directory
        // After: cd /path/to/directory && alanos create my-project
        ...
        try {
          ...
          const result = bunExecSync('alanos create migrated-project --yes', {
            encoding: 'utf8',
            timeout: TEST_TIMEOUTS.PROJECT_CREATION,
          }) as string;
  ...
  it(
    'does not hoist PGLITE database to parent .alan directory',
    async () => {
      // Create a parent directory that looks like an alan project
      const parentDir = await mkdtemp(join(tmpdir(), 'alan-parent-'));
      const parentalanDir = join(parentDir, '.alan');
      await mkdir(parentalanDir, { recursive: true });

      // Create a marker file to verify we're not using parent's .alan
      await writeFile(join(parentalanDir, 'parent-marker.txt'), 'parent');
      ...
      try {
        ...
        const result = bunExecSync('alanos create test-no-hoist --yes', {
          encoding: 'utf8',
          timeout: TEST_TIMEOUTS.PROJECT_CREATION,
        }) as string;
        ...
        // Verify project has its own .alan directory
        const projectalanDir = join('test-no-hoist', '.alan');
        expect(existsSync(projectalanDir)).toBe(true);

        // Verify the project's .alan directory has its own database directory
        const projectDbDir = join(projectalanDir, '.alandb');
        expect(existsSync(projectDbDir)).toBe(true);

        // Verify the parent marker file is NOT in the project's .alan directory
        const projectMarkerPath = join(projectalanDir, 'parent-marker.txt');
        ...
        // Verify it points to the project's .alan/.alandb, not parent's
        expect(pgliteDataDir).toContain(join('test-no-hoist', '.alan', '.alandb'));
        // Ensure it's not pointing to parent's .alan directory (without the project name)
        const sep = path.sep.replace(/\\/g, '\\\\'); // Escape backslashes for regex
        expect(pgliteDataDir).not.toMatch(
          new RegExp(`alan-parent-[^${sep}]+${sep}\\.alan${sep}\\.alandb$`)
        );
      } finally {
        process.chdir(originalDir);
        // Cleanup
