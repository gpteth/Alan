...
describe('alanOS Dev Commands', () => {
  ...
  beforeAll(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'alan-test-dev-'));
    ...
    await writeFile(
      ...
      JSON.stringify(
        {
          name: 'test-alanos-project',
          version: '1.0.0',
          type: 'module',
          dependencies: {
            '@alanos/core': '^1.0.0',
            '@alanos/server': '^1.0.0',
            '@alanos/plugin-sql': '^1.0.0',
  ...
  afterAll(async () => {
    ...
    if (testTmpDir && testTmpDir.includes('alan-test-dev-')) {
      try {
  ...
  const startDevAndWait = async (
    ...
    await mkdir(join(testTmpDir, 'alandb'), { recursive: true });

    const commandStr = `alanos dev ${args}`;
    ...
    try {
      const devProcess = Bun.spawn(['alanos', 'dev', ...args.split(' ')], {
        cwd: cwd || projectDir,
        env: {
          ...process.env,
          LOG_LEVEL: 'error',
          PGLITE_DATA_DIR: join(testTmpDir, 'alandb'),
  ...
  it('dev --help shows usage', () => {
    const result = bunExecSync(`alanos dev --help`, { encoding: 'utf8' });
    expect(result).toContain('Usage: alanos dev');
  ...
  it('dev command detects project type correctly', async () => {
    ...
    console.log(`[DEBUG] Command: alanos dev --port ${testServerPort}`);

    let devProcess: any;
    try {
      devProcess = Bun.spawn(['alanos', 'dev', '--port', testServerPort.toString()], {
        cwd: projectDir,
        env: {
          ...process.env,
          LOG_LEVEL: 'info',
    ...
    if (output && output.length > 0) {
      expect(output).toMatch(
        /(alanOS project|project mode|Identified as|Starting|development|dev mode|project|error|info)/i
      );
    } else {
  ...
  it('dev command handles non-alanos directory gracefully', async () => {
    // Create a non-alanOS project directory
    const nonalanDir = join(testTmpDir, 'non-alanos');
    await mkdir(nonalanDir, { recursive: true });
    await writeFile(
      join(nonalanDir, 'package.json'),
      JSON.stringify({ name: 'not-alanos', version: '1.0.0' })
    );

    let output = '';
    let outputReceived = false;

    // Use Bun.spawn for non-alan test
    console.log(`[DEBUG] Using Bun.spawn for non-alan test`);
    console.log(`[DEBUG] Command: alanos dev --port ${testServerPort}`);
    ...
    try {
      ...
      console.error(`[ERROR] Failed to spawn non-alan test:`, spawnError);
      console.error(`[ERROR] Platform: ${process.platform}`);
      console.error(`[ERROR] Working directory: ${nonalanDir}`);
      throw spawnError;
    }

    if (!devProcess || !devProcess.pid) {
      console.error('[ERROR] Failed to spawn dev process for non-alan test');
      console.error(`[ERROR] Command: alanos dev --port ${testServerPort}`);
      console.error(`[ERROR] Working directory: ${nonalanDir}`);
      throw new Error('Failed to spawn dev process');
    }
    ...
    const outputPromise = new Promise<void>((resolve) => {
      ...
      const handleStream = async (
        ...
        try {
          while (true) {
            ...
            const text = decoder.decode(value, { stream: true });
            output += text;
            console.log(`[NON-alan DIR ${streamName}] ${text}`);
      ...
      Promise.all([
        handleStream(devProcess.stdout, 'STDOUT'),
        handleStream(devProcess.stderr, 'STDERR'),
      ]).catch((err) => console.error('[NON-alan DIR TEST] Stream error:', err));

      // Fallback timeout
      setTimeout(() => {
        if (!outputReceived) {
          console.log('[NON-alan DIR TEST] No output received, resolving anyway');
        }
        resolve();
      }, TEST_TIMEOUTS.MEDIUM_WAIT);
    });

    // Wait for process to start and detect non-alanOS directory
    await outputPromise;

    console.log(`[NON-alan DIR TEST] Final output: "${output}"`);

    // More flexible pattern matching for non-alanOS detection
    // In CI, we primarily care that the process starts successfully
    expect(devProcess.pid).toBeDefined();
    ...
    if (output && output.length > 0) {
      expect(output).toMatch(
        /(not.*recognized|standalone mode|not.*alanOS|non.*alan|external|independent|error|info|Starting)/i
      );
    } else {
      console.log('[NON-alan DIR TEST] No output but process started successfully');
    }
  ...
  it('dev command validates port parameter', () => {
    ...
    try {
      bunExecSync(`alanos dev --port abc`, {
        encoding: 'utf8',
        stdio: 'pipe',
  ...
  it('dev command handles port conflicts by finding next available port', async () => {
    // Ensure alandb directory exists
    await mkdir(join(testTmpDir, 'alandb'), { recursive: true });
    ...
    try {
      ...
      const devProcess = Bun.spawn(['alanos', 'dev'], {
        cwd: projectDir,
        env: {
          ...process.env,
          FORCE_COLOR: '0',
  ...
  it('dev command uses specified port when provided', async () => {
    ...
    const helpResult = bunExecSync(`alanos dev --help`, { encoding: 'utf8' });
    ...
    const devProcess = Bun.spawn(['alanos', 'dev', '--port', specifiedPort.toString()], {
      cwd: projectDir,
      env: {
        ...process.env,
        FORCE_COLOR: '0',
