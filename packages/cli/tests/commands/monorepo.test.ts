...
describe('alanOS Monorepo Commands', () => {
  ...
  it('monorepo --help shows usage', async () => {
    const result = bunExecSync('alanos monorepo --help', { encoding: 'utf8' });
  ...
  it('monorepo fails when directory is not empty', async () => {
    ...
    try {
      ...
      bunExecSync('alanos monorepo --dir not-empty-dir', { encoding: 'utf8' });
