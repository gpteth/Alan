...
describe('alanOS Publish Commands', () => {
  ...
  beforeEach(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'alan-test-publish-'));
    ...
    // Mock alanOS data directory to avoid credential prompts
    const alanosDataDir = join(testTmpDir, '.alanos');
    process.env.alanOS_DATA_DIR = alanosDataDir;
    await mkdir(alanosDataDir, { recursive: true });

    // Create mock credentials file
    await writeFile(
      join(alanosDataDir, 'credentials.json'),
      JSON.stringify({
        github: {
    ...
    await writeFile(
      join(alanosDataDir, 'registry.json'),
      JSON.stringify({
        registryUrl: 'https://github.com/alanos/registry',
...
case "$1" in
  ...
  *)
  ...
  afterEach(async () => {
    ...
    delete process.env.alanOS_DATA_DIR;

    if (testTmpDir && testTmpDir.includes('alan-test-publish-')) {
      try {
  ...
  it('publish --help shows usage', () => {
    const result = bunExecSync(`alanos publish --help`, { encoding: 'utf8' });
    expect(result).toContain('Usage: alanos publish');
  ...
  it('publish command integrates with CLI properly', () => {
    ...
    const helpResult = bunExecSync(`alanos --help`, { encoding: 'utf8' });
    expect(helpResult).toContain('publish');

    // Test that publish command can be invoked
    const publishHelpResult = bunExecSync(`alanos publish --help`, { encoding: 'utf8' });
  ...
  it('publish dry-run flag works', () => {
    ...
    const result = bunExecSync(`alanos publish --dry-run --help`, { encoding: 'utf8' });
    expect(result).toContain('dry-run');
  });
});
