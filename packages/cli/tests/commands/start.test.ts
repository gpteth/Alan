...
describe('alanOS Start Commands', () => {
  let testTmpDir: string;
  let alanosPath: string;
  let originalCwd: string;
  let testServerPort: number;
  let processManager: TestProcessManager;
  let originalalanTestMode: string | undefined;

  beforeEach(async () => {
    // Store original working directory
    originalCwd = process.cwd();

    // Store original alan_TEST_MODE
    originalalanTestMode = process.env.alan_TEST_MODE;
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'alan-test-start-'));

    // Setup CLI path
    const scriptDir = join(__dirname, '..');
    alanosPath = join(scriptDir, '../dist/index.js');
    ...
    process.env.alan_TEST_MODE = 'true';
    process.env.BUN_TEST = 'true';

    // Ensure these flags are available for all spawned processes
    process.env.alan_CLI_TEST_MODE = 'true';
  });
  ...
  afterEach(async () => {
    ...
    delete process.env.alan_TEST_MODE;
    delete process.env.BUN_TEST;
    delete process.env.alan_CLI_TEST_MODE;

    // Restore original alan_TEST_MODE
    if (originalalanTestMode !== undefined) {
      process.env.alan_TEST_MODE = originalalanTestMode;
    } else {
      delete process.env.alan_TEST_MODE;
    }
    ...
    if (testTmpDir && testTmpDir.includes('alan-test-start-')) {
      try {
  ...
  it('start command shows help', async () => {
    const { stdout: result } = await bunExecSimple('bun', [alanosPath, 'start', '--help'], {
    ...
    expect(result).toContain('Usage: alanos start');
        ...
        for (let i = 0; i < maxRetries; i++) {
          try {
            ...
            const { stdout } = await bunExecSimple(
              'bun',
              [alanosPath, 'agent', 'list', '--remote-url', `http://localhost:${testServerPort}`],
              {
                timeout: TEST_TIMEOUTS.STANDARD_COMMAND,
                env: {
                  ...process.env,
  ...
  it(
    ...
    async () => {
      ...
      await mkdir(join(testTmpDir, 'alandb2'), { recursive: true });
      ...
      const serverProcess = processManager.spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            LOG_LEVEL: 'debug',
            PGLITE_DATA_DIR: join(testTmpDir, 'alandb2'),
            NODE_ENV: 'test',
            alan_TEST_MODE: 'true',
            BUN_TEST: 'true',
            alan_CLI_TEST_MODE: 'true',
  ...
  it('multiple character formats parse', async () => {
    ...
    for (const fmt of formats) {
      const { stdout: result } = await bunExecSimple(
        'bun',
        [alanosPath, 'start', '--character', `${adaPath}${fmt}${adaPath}`, '--help'],
        {
          timeout: TEST_TIMEOUTS.STANDARD_COMMAND,
          env: {
            ...process.env,
  ...
  it('graceful acceptance of invalid character file list (dry)', async () => {
    ...
    const { stdout: result } = await bunExecSimple(
      'bun',
      [alanosPath, 'start', '--character', `${adaPath},does-not-exist.json`, '--help'],
      {
        timeout: TEST_TIMEOUTS.STANDARD_COMMAND,
        env: {
          ...process.env,
  ...
  it(
    ...
    async () => {
      ...
      await mkdir(join(testTmpDir, 'alandb3'), { recursive: true });
      ...
      const serverProcess = processManager.spawn(
        ...
        [join(__dirname, '..', '../dist/index.js'), 'start', '--configure', '--character', adaPath],
        {
          env: {
            ...process.env,
            LOG_LEVEL: 'debug',
            PGLITE_DATA_DIR: join(testTmpDir, 'alandb3'),
            NODE_ENV: 'test',
            alan_TEST_MODE: 'true',
            BUN_TEST: 'true',
            alan_CLI_TEST_MODE: 'true',
