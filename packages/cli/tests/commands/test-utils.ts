...
export async function setupTestEnvironment(): Promise<TestContext> {
  ...
  const testTmpDir = await mkdtemp(join(tmpdir(), 'alan-test-'));
...
export async function cleanupTestEnvironment(context: TestContext): Promise<void> {
  ...
  if (context.testTmpDir && context.testTmpDir.includes('alan-test-')) {
    try {
...
/**
 * Helper to create a basic alanOS project for testing
 */
export async function createTestProject(projectName: string): Promise<void> {
  ...
  try {
    const result = await bunExecSimple(`alanos create ${projectName} --yes`);
...
export function expectHelpOutput(
  ...
  if (!output.includes(`Usage: alanos ${command}`)) {
    throw new Error(`Expected help output to contain 'Usage: alanos ${command}', got: ${output}`);
  }
  ...
  while (Date.now() - startTime < maxWaitTime) {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:${port}${endpoint}`, {
        ...
        headers: {
          'User-Agent': 'alanOS-Test-Client/1.0',
