...
describe('alanOS Update Commands', () => {
  ...
  beforeEach(async () => {
    ...
    testTmpDir = await mkdtemp(join(tmpdir(), 'alan-test-update-'));
  ...
  afterEach(async () => {
    ...
    if (testTmpDir && testTmpDir.includes('alan-test-update-')) {
      try {
  ...
  const makeProj = async (name: string) => {
    bunExecSync(`alanos create ${name} --yes`, { encoding: 'utf8' });
    process.chdir(join(testTmpDir, name));
  };

  // --help
  it('update --help shows usage and options', async () => {
    const result = bunExecSync('alanos update --help', { encoding: 'utf8' });
    expect(result).toContain('Usage: alanos update');
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('alanos update', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('alanos update --check', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('alanos update --skip-build', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('alanos update --packages', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      const result = bunExecSync('alanos update --cli', { encoding: 'utf8' });
  ...
  it(
    ...
    async () => {
      ...
      const result = bunExecSync('alanos update --cli --packages', { encoding: 'utf8' });
  ...
  it.skipIf(process.env.CI === 'true' || process.env.GITHUB_ACTIONS === 'true')(
    ...
    async () => {
      ...
      expect(result).toMatch(
        /(Project successfully updated|Update completed|already up to date|No updates available|create a new alanOS project|This appears to be an empty directory|Version: monorepo|Version: 1\.2\.\d+)/
      );
    },
    TEST_TIMEOUTS.STANDARD_COMMAND
  );
  ...
  it(
    ...
    async () => {
      ...
      expect(result).toContain("This directory doesn't appear to be an alanOS project");
    },
    TEST_TIMEOUTS.STANDARD_COMMAND
  );

  it(
    'update --packages shows helpful message in non-alanos project',
    async () => {
      // Create a non-alanOS package.json
      await writeFile(
        'package.json',
        JSON.stringify(
          {
      ...
      expect(result).toContain('alanos create');
    },
    TEST_TIMEOUTS.STANDARD_COMMAND
  );

  it(
    'update --packages works in alanos project with dependencies',
    async () => {
      await makeProj('update-alanos-project');

      // Add some alanOS dependencies to make it a valid project
      await writeFile(
        'package.json',
        JSON.stringify(
          {
      ...
      const result = bunExecSync('alanos update --packages --check', { encoding: 'utf8' });

      expect(result).toContain('alanOS');
    },
    TEST_TIMEOUTS.INDIVIDUAL_TEST
  );

  it(
    'update --packages shows message for project without alanos dependencies',
    async () => {
      await makeProj('update-no-deps-project');

      // Create package.json without alanOS dependencies
      await writeFile(
        'package.json',
        JSON.stringify(
          {
      ...
      expect(result).toContain('No alanOS packages found');
    },
    TEST_TIMEOUTS.INDIVIDUAL_TEST
  );
  ...
  it.skipIf(process.env.CI === 'true' || process.env.GITHUB_ACTIONS === 'true')(
    ...
    async () => {
      ...
      const tmpDir = mkdtempSync(join(tmpdir(), 'alan-test-'));
  ...
  describe('bunx/npx detection', () => {
    it.skip('update --cli shows warning when running via bunx', () => {
      ...
      const result = bunExecSync('alanos update --cli', {
        encoding: 'utf8',
        env: {
          ...process.env,
      ...
      expect(result).toContain('bun install -g @alanos/cli');
    });
    ...
    it.skip('update --cli works with global bun installation', () => {
      ...
      process.argv = [
        ...
        '/Users/user/.bun/install/global/@alanos/cli/dist/index.js',
        'update',
        '--cli',
      ];
      process.env = {};
    ...
    it.skip('update --cli works with global npm installation', () => {
      ...
      process.argv = [
        'node',
        '/usr/local/lib/node_modules/@alanos/cli/dist/index.js',
        'update',
        '--cli',
      ];
      process.env = {};
    ...
    it(
      ...
      async () => {
        ...
        const result = bunExecSync('alanos update --packages', {
          encoding: 'utf8',
          env: {
            ...process.env,
            BUN_INSTALL_CACHE_DIR: '/Users/user/.bun/install/cache',
    ...
    it.skip(
      ...
      async () => {
        ...
        // Simulate bunx execution by setting environment variable
        const result = bunExecSync('alanos update', {
          encoding: 'utf8',
          env: {
            ...process.env,
            BUN_INSTALL_CACHE_DIR: '/Users/user/.bun/install/cache',
        ...
        expect(result).toContain('CLI update is not available when running via npx or bunx');
        expect(result).toMatch(
          /(Project successfully updated|Update completed|already up to date|No updates available|Found.*alanOS package)/
        );
      },
      TEST_TIMEOUTS.INDIVIDUAL_TEST
    );
