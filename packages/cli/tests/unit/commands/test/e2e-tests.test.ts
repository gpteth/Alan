...
mock.module('../../../../src/characters/alan', () => ({
  getalanCharacter: mock().mockImplementation(() => ({
    name: 'alan',
    bio: 'Test character',
  })),
}));
...
describe('E2E Tests Plugin Isolation', () => {
  let originalalanTestingPlugin: string | undefined;
  let runE2eTests: any;
  let loadProject: any;
  ...
  beforeEach(async () => {
    // Save original alan_TESTING_PLUGIN value specifically
    originalalanTestingPlugin = process.env.alan_TESTING_PLUGIN;
  ...
  afterEach(() => {
    ...
    if (originalalanTestingPlugin !== undefined) {
      process.env.alan_TESTING_PLUGIN = originalalanTestingPlugin;
    } else {
      delete process.env.alan_TESTING_PLUGIN;
    }
  });
  ...
  describe('Plugin Test Environment Variable', () => {
    it('should set alan_TESTING_PLUGIN=true when testing a plugin', async () => {
      ...
      Object.defineProperty(process.env, 'alan_TESTING_PLUGIN', {
      ...
      try {
        ...
        if (originalDescriptor) {
          Object.defineProperty(process.env, 'alan_TESTING_PLUGIN', originalDescriptor);
        } else {
          delete process.env.alan_TESTING_PLUGIN;
        }
      }
    ...
    it('should not set alan_TESTING_PLUGIN when testing a project', async () => {
      ...
      expect(process.env.alan_TESTING_PLUGIN).toBeUndefined();
    ...
    it('should clean up alan_TESTING_PLUGIN after tests complete', async () => {
      // Setup mock to return a plugin using test fixtures
      loadProject.mockResolvedValue(TestFixtures.mockPluginProject);
      ...
      TestRunnerMock.mockImplementation(() => ({
        runTests: mock().mockImplementation(() => {
          ...
          envDuringTest = process.env.alan_TESTING_PLUGIN;
    ...
    it('should clean up alan_TESTING_PLUGIN even if tests fail', async () => {
  ...
  describe('Plugin vs Project Detection', () => {
    ...
    it('should correctly identify and test projects', async () => {
      ...
      const projectInfo = {
        ...TestFixtures.mockDirectoryInfo.project,
        alanPackageCount: 3, // Override for this test
      };
