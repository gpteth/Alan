...
import type { IAgentRuntime, Plugin, ProjectAgent, Character } from '@alanos/core';

// Mock the logger
mock.module('@alanos/core', async (importOriginal) => {
...
describe('TestRunner Plugin Isolation', () => {
  ...
  const originalEnv = process.env.alan_TESTING_PLUGIN;
  ...
  afterEach(() => {
    ...
    if (originalEnv) {
      process.env.alan_TESTING_PLUGIN = originalEnv;
    } else {
      delete process.env.alan_TESTING_PLUGIN;
    }
  });
  ...
  describe('Plugin Identification', () => {
    it('should identify plugin under test when alan_TESTING_PLUGIN is set', () => {
      process.env.alan_TESTING_PLUGIN = 'true';

      const sqlPlugin: Plugin = {
        name: '@alanos/plugin-sql',
      ...
      const projectAgent: ProjectAgent = {
        character: { name: 'alan', bio: 'Test bio' } as Character,
    ...
    it('should not identify as plugin test when alan_TESTING_PLUGIN is not set', () => {
