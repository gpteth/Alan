/**
 * Centralized test utilities for alanOS CLI tests
 *
...
/**
 * Run alanOS CLI command synchronously
 * Specialized helper for testing CLI commands
 *
 * @param args - CLI arguments (without 'alanos' prefix)
 * @param options - Execution options
...
export function runCliCommandSync(args: string[], options: BunExecSyncOptions = {}): string {
  // Use global alanos command
  const command = `alanos ${args.map((arg) => (arg.includes(' ') ? `"${arg}"` : arg)).join(' ')}`;
