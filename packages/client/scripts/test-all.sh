...
echo -e "${BLUE}🧪 alanOS Complete Test Suite${NC}"
echo "=================================="
...
# 3. alanOS Core Tests - Skip in CI as they're run separately
if [ "$CI" = "true" ]; then
  echo -e "\n${YELLOW}🤖 Skipping alanOS Core Tests (run separately in CI)...${NC}"
else
  echo -e "\n${YELLOW}🤖 Running alanOS Core Tests...${NC}"
  cd ../.. && bun test
  if [ $? -ne 0 ]; then
    echo -e "${RED}❌ alanOS core tests failed${NC}"
    FAILED=1
  else
    echo -e "${GREEN}✅ alanOS core tests passed${NC}"
  fi
  cd packages/client
fi
...
cd ../..
bun run start > /tmp/alanos-server.log 2>&1 &
SERVER_PID=$!
cd packages/client
...
npx vite --port 5173 > /tmp/alanos-client.log 2>&1 &
CLIENT_PID=$!
...
if [ $FAILED -eq 0 ]; then
  ...
  echo -e "${GREEN}   ✓ alanOS core tests${NC}"
