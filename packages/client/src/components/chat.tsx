...
import type { Agent, Media, UUID } from '@alanos/core';
import {
  AgentStatus,
  ChannelType,
...
export default function Chat({
    ...
    async (agentIdForNewChannel: UUID | undefined) => {
      ...
      if (latestChannel) {
        try {
          ...
          const latestMessages = await alanClient.messaging.getChannelMessages(latestChannel.id, {
            limit: 30,
          });
  ...
  const handleDeleteCurrentDmChannel = useCallback(() => {
    ...
    confirm(
      ...
      async () => {
        ...
        try {
          const alanClient = createalanClient();
          await alanClient.messaging.deleteChannel(channelToDelete.id);
  ...
  const updateChatTitle = async () => {
    ...
    const alanClient = createalanClient();
    const data = await alanClient.messaging.generateChannelTitle(
      finalChannelIdForHooks,
      contextId
    );

    const title = data?.title;
    const participants = await alanClient.messaging.getChannelParticipants(
      chatState.currentDmChannelId
    );
    if (title && participants) {
      ...
      await alanClient.messaging.updateChannel(finalChannelIdForHooks, {
        name: title,
                    ...
                    onClick: () => {
                      ...
                      confirm(
                        ...
                        async () => {
                          try {
                            const alanClient = createalanClient();
                            await alanClient.messaging.deleteChannel(finalChannelIdForHooks);
                            toast({
                              title: 'Group Deleted',
