  ...
  useEffect(() => {
    ...
    if (
      ...
      toast({
        ...
        description: (
          <div className="flex items-center gap-2">
            ...
            <span>Successfully reconnected to the alan server.</span>
          </div>
        ),
      });
      ...
      toast({
        ...
        description: 'Attempting to reconnect to the alan server...',
        variant: 'destructive',
      });
    }
        ...
        {showingError && (
          <TooltipContent side="top" align="center" className="max-w-xs">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs">Please ensure the alan server is running and accessible.</p>
              {!isUnauthorized && (
                <p className="text-xs">Try refreshing the connection or check server logs.</p>
              )}
              {isUnauthorized && (
                <p className="text-xs">
                  Check the X-API-KEY configured in your client or the alan_SERVER_AUTH_TOKEN on
                  the server.
                </p>
              )}
            </div>
