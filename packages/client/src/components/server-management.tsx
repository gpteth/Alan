...
import type { UUID } from '@alanos/core';
...
export function ServerManagement({ open, onOpenChange }: ServerManagementProps) {
    ...
    const loadServerAgents = async () => {
      ...
      for (const server of serversData.data.servers) {
        try {
          ...
          const response = await alanClient.agents.getAgentsForServer(server.id);
          if (response.success) {
  ...
  const handleAddAgentToServer = async () => {
    ...
    try {
      const alanClient = createalanClient();
      await alanClient.agents.addAgentToServer(selectedServerId, selectedAgentId);
  ...
  const handleRemoveAgentFromServer = async (serverId: UUID, agentId: UUID) => {
    ...
    try {
      const alanClient = createalanClient();
      await alanClient.agents.removeAgentFromServer(serverId, agentId);
