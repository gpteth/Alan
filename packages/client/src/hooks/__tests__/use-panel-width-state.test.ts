...
describe('usePanelWidthState', () => {
  ...
  it('should load persisted values from localStorage on mount', async () => {
    ...
    localStorage.setItem('alan-main-panel-width', '65');
    localStorage.setItem('alan-sidebar-panel-width', '35');
  ...
  it('should persist main panel size to localStorage when updated', async () => {
    ...
    expect(localStorage.getItem('alan-main-panel-width')).toBe('75');
  });
  ...
  it('should persist sidebar panel size to localStorage when updated', async () => {
    ...
    expect(localStorage.getItem('alan-sidebar-panel-width')).toBe('25');
  });
  ...
  it('should reset panel sizes to defaults and clear localStorage', () => {
    ...
    localStorage.setItem('alan-main-panel-width', '60');
    localStorage.setItem('alan-sidebar-panel-width', '40');
    ...
    expect(result.current.mainPanelSize).toBe(70);
    expect(result.current.sidebarPanelSize).toBe(30);
    expect(localStorage.getItem('alan-main-panel-width')).toBe(null);
    expect(localStorage.getItem('alan-sidebar-panel-width')).toBe(null);
  });

  it('should handle localStorage errors gracefully when reading', () => {
    // Set up invalid values
    localStorage.setItem('alan-main-panel-width', 'invalid');
    localStorage.setItem('alan-sidebar-panel-width', 'invalid');
  ...
  it('should handle invalid JSON values from localStorage', () => {
    localStorage.setItem('alan-main-panel-width', 'not-a-number');
    localStorage.setItem('alan-sidebar-panel-width', 'also-not-a-number');
  ...
  it('should handle out-of-range values from localStorage', () => {
    localStorage.setItem('alan-main-panel-width', '150'); // Way too high (> 100)
    localStorage.setItem('alan-sidebar-panel-width', '0'); // Way too low (<= 0)
  ...
  it('should update floating threshold and persist to localStorage', () => {
    ...
    expect(localStorage.getItem('alan-floating-threshold')).toBe('900');
  });
