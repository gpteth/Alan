import { Character, Content } from '@alanos/core';
import { usePlugins } from '@/hooks/use-plugins';

const PROVIDER_PLUGIN_MAPPINGS: Record<string, string> = {
  google: '@alanos/plugin-google-genai',
  llama_local: '@alanos/plugin-ollama',
  // extend as needed
};
...
const ESSENTIAL_PLUGINS = ['@alanos/plugin-sql', '@alanos/plugin-bootstrap'];
...
export function useConvertCharacter() {
  ...
  const matchPlugins = (v1: V1Character): string[] => {
    ...
    if (Array.isArray(v1.clients)) {
      for (const client of v1.clients) {
        ...
        if (mapped && availablePlugins.includes(mapped)) {
          ...
          const constructed = `@alanos/plugin-${lower}`;
    ...
    if (typeof v1.modelProvider === 'string') {
      ...
      if (!providerMatched) {
        if (availablePlugins.includes('@alanos/plugin-openai')) {
          matched.add('@alanos/plugin-openai');
        }
      }
    } else {
