...
// Create alanClient instance
const alanClient = createalanClient();
import { type UUID, ChannelType } from '@alanos/core';
...
export function useGetOrCreateDmChannel() {
  ...
  return useMutation({
    mutationFn: async (targetUserId: UUID) => {
      ...
      const alanClient = createalanClient();
      const result = await alanClient.messaging.getOrCreateDmChannel({
...
export function useCreateDmChannel() {
  ...
  return useMutation({
    mutationFn: async ({ agentId, channelName }: { agentId: UUID; channelName: string }) => {
      ...
      const alanClient = createalanClient();
      const result = await alanClient.messaging.createGroupChannel({
      ...
      return result; // Direct result from alanClient
    },
