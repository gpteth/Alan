...
const REGISTRY_ORG = 'alanos-plugins';
...
const CORE_PLUGINS = ['@alanos/plugin-bootstrap', '@alanos/plugin-sql'];
...
/**
 * Convert plugin name for registry lookup - handles both @alanos and @alanos-plugins formats
 */
function getRegistryPluginName(pluginName: string): string {
  // If already in @alanos-plugins format, return as is
  if (pluginName.startsWith('@alanos-plugins/')) {
    return pluginName;
  }
  // Convert @alanos to @alanos-plugins format
  return pluginName.replace('@alanos/', '@alanos-plugins/');
}
...
function getGitHubRepoPath(
  ...
  if (repoMatch) {
    ...
    // Try to parse as a simple owner/repo format (e.g., "alanos-plugins/plugin-google-genai")
...
async function fetchPluginPackageJson(
  ...
  const packageName = pluginName.replace('@alanos/', '');
...
function extractRequiredSecrets(
  ...
  // Try to get secrets from alanos.secrets
  if (packageJson?.alanos?.secrets) {
    return packageJson.alanos.secrets.filter((secret) => secret.required);
  }

  // Legacy format - convert string array to PluginSecret array
  if (packageJson?.alanos?.requiredSecrets) {
    return packageJson.alanos.requiredSecrets.map((secretName) => ({
      name: secretName,
      required: true,
    }));
  }
