    ...
    queryFn: async () => {
      try {
        ...
        const registryPlugins = Object.entries(registryData.registry || {})
          ...
          .map(([name]) => name.replace(/^@alanos-plugins\//, '@alanos/'))
          .sort();
        ...
        try {
          if (agentsResponse?.length > 0) {
            ...
            if (activeAgent && activeAgent.id) {
              const agentDetailResponse = await alanClient.agents.getAgent(activeAgent.id);
        ...
        return [
          ...
          '@alanos/plugin-elevenlabs',
          '@alanos/plugin-anthropic',
          '@alanos/plugin-browser',
