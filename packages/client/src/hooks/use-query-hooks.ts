...
import { createalanClient } from '@/lib/api-client-config';
import type { Agent, Content, Memory, UUID, Memory as CoreMemory } from '@alanos/core';
import {
  useQuery,
  useMutation,
  useQueryClient,
...
// Create alanClient instance for direct API calls
const alanClient = createalanClient();
...
export function useAgents(options = {}) {
  ...
  return useQuery<{ data: { agents: Partial<AgentWithStatus>[] } }>({
    queryKey: ['agents'],
    queryFn: async () => {
      const result = await alanClient.agents.listAgents();
      return { data: result };
    },
...
export function useAgent(agentId: UUID | undefined | null, options = {}) {
  ...
  return useQuery<{ data: AgentWithStatus }>({
    ...
    queryFn: async () => {
      ...
      const result = await alanClient.agents.getAgent(agentId);
      return { data: result };
    },
    staleTime: STALE_TIMES.FREQUENT, // Use shorter stale time for real-time data
...
export function useStartAgent() {
  ...
  return useMutation<{ data: { id: UUID; name: string; status: string } }, Error, UUID>({
    mutationFn: async (agentId: UUID) => {
      try {
        const result = await alanClient.agents.startAgent(agentId);
...
export function useStopAgent() {
  ...
  return useMutation<{ data: { message: string } }, Error, UUID>({
    mutationFn: async (agentId: UUID) => {
      const result = await alanClient.agents.stopAgent(agentId);
...
export function useChannelMessages(
  ...
  const fetchMessages = useCallback(
    async (beforeTimestamp?: number) => {
      ...
      try {
        const response = await alanClient.messaging.getChannelMessages(channelId, {
          limit: 30,
...
export function useAgentActions(agentId: UUID, roomId?: UUID, excludeTypes?: string[]) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await alanClient.agents.getAgentLogs(agentId, {
        roomId,
        count: 50,
        excludeTypes,
      });
...
export function useDeleteLog() {
  ...
  return useMutation({
    mutationFn: async ({ agentId, logId }: { agentId: UUID; logId: UUID }) => {
      await alanClient.agents.deleteAgentLog(agentId, logId);
...
export function useAgentMemories(
  ...
  return useQuery({
    queryKey,
    queryFn: async () => {
      const result = await alanClient.memory.getAgentMemories(agentId, {
        roomId: channelId,
        tableName,
        includeEmbedding,
      });
...
export function useDeleteMemory() {
  ...
  return useMutation({
    mutationFn: async ({ agentId, memoryId }: { agentId: UUID; memoryId: UUID }) => {
      await alanClient.memory.deleteMemory(agentId, memoryId);
...
export function useDeleteAllMemories() {
  ...
  return useMutation({
    mutationFn: async ({ agentId, roomId }: { agentId: UUID; roomId: UUID }) => {
      await alanClient.memory.clearRoomMemories(agentId, roomId);
      return { agentId };
    },
    onSuccess: (data) => {
      // Invalidate relevant queries to trigger refetch
...
export function useUpdateMemory() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const result = await alanClient.memory.updateMemory(agentId, memoryId, memoryData);
...
export function useDeleteGroupMemory() {
  ...
  return useMutation({
    mutationFn: async ({ serverId, memoryId }: { serverId: UUID; memoryId: UUID }) => {
      await alanClient.messaging.deleteMessage(serverId, memoryId);
      return { serverId };
    },
    onSuccess: ({ serverId }) => {
...
export function useClearGroupChat() {
  ...
  return useMutation({
    mutationFn: async (serverId: UUID) => {
      await alanClient.messaging.clearChannelHistory(serverId);
      return { serverId };
    },
    onSuccess: ({ serverId }) => {
      queryClient.invalidateQueries({ queryKey: ['groupmessages', serverId] });
...
export function useAgentPanels(agentId: UUID | undefined | null, options = {}) {
  ...
  return useQuery<{
    ...
    queryFn: async () => {
      ...
      const result = await alanClient.agents.getAgentPanels(agentId);
...
export function useAgentsWithDetails(): AgentsWithDetailsResult {
  ...
  const agentQueries = useQueries<UseQueryResult<{ data: Agent }, Error>[]>({
    queries: agentIds.map((id) => ({
      ...
      queryFn: async () => {
        const result = await alanClient.agents.getAgent(id);
        return { data: result };
      },
...
export function useAgentInternalMemories(
  ...
  return useQuery<CoreMemory[], Error>({
    ...
    queryFn: async () => {
      ...
      const response = await alanClient.memory.getAgentInternalMemories(
        agentId,
        agentPerspectiveRoomId,
        includeEmbedding
      );
...
export function useDeleteAgentInternalMemory() {
  ...
  return useMutation<{ agentId: UUID; memoryId: UUID }, Error, { agentId: UUID; memoryId: UUID }>({
    mutationFn: async ({ agentId, memoryId }: { agentId: UUID; memoryId: UUID }) => {
      await alanClient.memory.deleteAgentInternalMemory(agentId, memoryId);
      return { agentId, memoryId };
    },
...
export function useDeleteAllAgentInternalMemories() {
  ...
  >({
    mutationFn: async ({ agentId, agentPerspectiveRoomId }) => {
      await alanClient.memory.deleteAllAgentInternalMemories(agentId, agentPerspectiveRoomId);
...
export function useUpdateAgentInternalMemory() {
  ...
  >({
    mutationFn: async ({ agentId, memoryId, memoryData }) => {
      const response = await alanClient.memory.updateAgentInternalMemory(
        agentId,
        memoryId,
        memoryData
      );
...
export function useServers(options = {}) {
  ...
  return useQuery<{ data: { servers: ClientMessageServer[] } }>({
    queryKey: ['servers'],
    queryFn: async () => {
      const result = await alanClient.messaging.listServers();
...
export function useChannels(serverId: UUID | undefined, options = {}) {
  ...
  return useQuery<{ data: { channels: ClientMessageChannel[] } }>({
    ...
    queryFn: async () => {
      ...
      const result = await alanClient.messaging.getServerChannels(serverId);
...
export function useChannelDetails(channelId: UUID | undefined, options = {}) {
  ...
  return useQuery<{ success: boolean; data: ClientMessageChannel | null }>({
    ...
    queryFn: async () => {
      ...
      const result = await alanClient.messaging.getChannelDetails(channelId);
...
export function useChannelParticipants(channelId: UUID | undefined, options = {}) {
  ...
  return useQuery<{ success: boolean; data: UUID[] }>({
    ...
    queryFn: async () => {
      ...
      try {
        const result = await alanClient.messaging.getChannelParticipants(channelId);
...
export function useDeleteChannelMessage() {
  ...
  >({
    mutationFn: async ({ channelId, messageId }) => {
      await alanClient.messaging.deleteMessage(channelId, messageId);
...
export function useClearChannelMessages() {
  ...
  return useMutation<{ channelId: UUID }, Error, UUID>({
    mutationFn: async (channelId: UUID) => {
      await alanClient.messaging.clearChannelHistory(channelId);
      return { channelId };
    },
...
export function useDeleteChannel() {
  ...
  return useMutation<void, Error, { channelId: UUID; serverId: UUID }>({
    mutationFn: async ({ channelId }) => {
      await alanClient.messaging.deleteChannel(channelId);
    },
    onSuccess: (_data, variables) => {
      toast({
        title: 'Group Deleted',
