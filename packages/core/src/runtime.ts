...
export class AgentRuntime implements IAgentRuntime {
  ...
  async initialize(): Promise<void> {
    ...
    if (!this.adapter) {
      this.logger.error(
        'Database adapter not initialized. Make sure @alanos/plugin-sql is included in your plugins.'
      );
      throw new Error(
        'Database adapter not initialized. The SQL plugin (@alanos/plugin-sql) is required for agent initialization. Please ensure it is included in your character configuration.'
      );
    }
    try {
