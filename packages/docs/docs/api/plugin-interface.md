...
This document provides the complete Plugin interface definition and all related types as implemented in the alanOS codebase.
...
````typescript
/**
 * Abstract base class for all alanOS services
 *
 ...
 *
 * Test suites group related tests together and are used by the alanOS
...
5. **Testing**: Use the `TestSuite` structure to organize plugin tests, which can be run via the alanOS test runner.

This interface provides a comprehensive foundation for creating modular, extensible plugins that integrate seamlessly with the alanOS agent runtime.
