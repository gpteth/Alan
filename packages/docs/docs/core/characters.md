...
description: Defining agent personalities and behavior with Character files in alanOS
...
Character files are the heart of an agent's personality in alanOS. They are JSON or TypeScript files that define everything from the agent's name and backstory to its conversational style and special abilities.

> **Source Reference**: The core `Character` type is defined in [`packages/core/src/types/agent.ts`](https://github.com/alanos/alan/blob/main/packages/core/src/types/agent.ts).
...
export const myCharacter: Character = {
  ...
  plugins: ['@alanos/plugin-discord', '@alanos/plugin-openai'],
    ...
    [
      {
        "name": "{{user1}}",
        "content": {
          "text": "Can you help me understand what alanOS is?"
        }
      },
      {
        "name": "alan",
        "content": {
          "text": "Of course! alanOS is a framework for building and deploying autonomous AI agents. It lets you create agents with distinct personalities that can interact across multiple platforms.",
          "actions": ["REPLY"]
        }
      }
    ]
