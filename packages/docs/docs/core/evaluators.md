...
description: Understanding alanOS evaluators - cognitive components that enable agents to learn and evolve
...
Evaluators are cognitive components in the alanOS framework that enable agents to process conversations, extract knowledge, and build understanding - similar to how humans form memories after interactions. They provide a structured way for agents to introspect, learn from interactions, and evolve over time.
...
```typescript
import { Handler, Validator, EvaluationExample } from '@alanos/core';
