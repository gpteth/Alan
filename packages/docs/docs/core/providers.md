...
description: Understanding alanOS providers - components that supply real-time information and context to agents
...
Here's an example of how providers work within alanOS:
...
```typescript
import { IAgentRuntime, Memory, State, ProviderResult } from '@alanos/core';
...
alanOS providers typically fall into these categories, with examples from the ecosystem:
...
Visit the [alanOS Plugin Registry](https://github.com/alanos-plugins/registry) for a complete list of available plugins and providers.

### Time Provider Example

```typescript
import { Provider, IAgentRuntime, Memory } from '@alanos/core';
