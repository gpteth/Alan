...
description: Common questions and answers about installing, configuring, and using alanOS
...
alan is an extensible open-source framework for building autonomous AI agents that can engage in natural conversations, learn from interactions, and maintain consistent personalities across platforms like Farcaster, X, Discord, and Telegram.

### What's the difference between v1 and v2?

V2 is a major upgrade that makes alan more powerful and easier to use. The main changes are:
...
### What are the system requirements for running alan?
...
### How do I use local models with alan?

To use local models with alan:

1. Install [Ollama](https://ollama.ai) on your system
2. Download your desired model (e.g., `ollama pull llama3.1`)
3. Install the Ollama plugin: `bun install @alanos-plugins/plugin-ollama`
4. Add the plugin to your character file:
   ```json
   {
     "plugins": ["@alanos-plugins/plugin-ollama"]
   }
   ```
...
For more details, see the [plugin-ollama documentation](https://github.com/alanos-plugins/plugin-ollama).

### How do I update alan to the latest version?

For CLI projects:

```bash
bun update -g @alanos/cli
```
...
```bash
alanos start --characters="characters/agent1.json,characters/agent2.json"
```
...
**⚠️ Breaking Change from v0.x to v1.x:** In alanOS v0.x, you could specify models directly in the character file using `modelProvider` and related fields. In v1.x, models are exclusively configured through plugins.
...
{
  "plugins": [
    "@alanos/plugin-anthropic", // Primary for text generation
    "@alanos/plugin-openai" // Fallback for embeddings
  ]
}
```
...
- `@alanos/plugin-openai`: Supports all model types (text, embeddings, objects)
- `@alanos/plugin-anthropic`: Text generation only
- `@alanos-plugins/plugin-ollama`: Local models via Ollama
- `@alanos/plugin-google-genai`: Google's Gemini models
...
- `@alanos/plugin-anthropic` requires `ANTHROPIC_API_KEY`
- `@alanos-plugins/plugin-ollama` requires Ollama configuration (see local models section)

---

## Memory and Knowledge Management

### How does memory management work in alanOS?

alanOS uses RAG (Retrieval-Augmented Generation) to convert prompts into vector embeddings for efficient context retrieval and memory storage.
...
- **To reset memory**:
  - For PGLite: Delete the `.alan/.alandb` folder and restart
  - For PostgreSQL: Drop and recreate the database (see "How do I clear or reset my agent's memory?" below)
- **To add documents**: Use the `@alanos/plugin-knowledge` plugin and place documents in the `knowledge/` or `docs/` folder
...
alanOS uses PGLite (local) or PostgreSQL (production) for data storage. There is currently no CLI command to clear memory, so you need to manually reset the database:

1. **For PGLite (local development)**:

   - Delete the `.alandb` folder in your project
   - Default location: `.alan/.alandb`
...
The easiest way is to use the **@alanos/plugin-knowledge** plugin:

1. **Install the plugin and an LLM provider that supports embeddings:**

   ```bash
   bun install @alanos/plugin-knowledge
   ```
...
2. **Add both plugins to your character file:**
   ...
   ```json
   {
     "plugins": [
       "@alanos/plugin-sql",
       "@alanos/plugin-openai", // or plugin-google-genai, plugin-ollama
...
- `@alanos/plugin-openai` - OpenAI embeddings (text-embedding-3-small)
- `@alanos-plugins/plugin-google-genai` - Google embeddings (text-embedding-004)
- `@alanos-plugins/plugin-ollama` - Local embeddings (nomic-embed-text)

The plugin automatically uses embeddings from your configured LLM provider. For more details, see the [plugin-knowledge documentation](https://github.com/alanOS/alan/tree/main/packages/plugin-knowledge).
...
Plugin order is **critical** in alanOS because plugins are loaded sequentially and may depend on services provided by earlier plugins. The wrong order can cause initialization failures or missing functionality.
...
{
  "plugins": [
    "@alanos/plugin-sql", // 1. MUST BE FIRST - provides database
    "@alanos/plugin-anthropic", // 2. Primary LLM provider
    "@alanos/plugin-openai", // 3. Fallback for embeddings (if needed)
    "@alanos/plugin-bootstrap" // 4. Core message handling
...
1. **Database Plugin (`@alanos/plugin-sql`)** - **MUST ALWAYS BE FIRST**
...
2. **LLM Provider Plugins** - Must come after database, before plugins that need AI
   ...
   - Examples: `@alanos/plugin-openai`, `@alanos/plugin-anthropic`, `@alanos/plugin-google-genai`
...
4. **Bootstrap Plugin (`@alanos/plugin-bootstrap`)** - Highly recommended
   ...
   - While technically optional (you could implement your own message handling), it provides the standard alanOS behavior that most users expect

5. **Feature Plugins** - Can go in any order after dependencies
   - Platform integrations: `@alanos/plugin-discord`, `@alanos/plugin-telegram`
   - Additional capabilities: `@alanos/plugin-knowledge`, `@alanos/plugin-image-generation`
...
{
  "plugins": [
    "@alanos/plugin-knowledge",  // Needs database and embeddings!
    "@alanos/plugin-sql",
...
{
  "plugins": [
    "@alanos/plugin-sql",         // Database first
    "@alanos/plugin-openai",      // Provides embeddings
    "@alanos/plugin-knowledge"    // Can now use both
  ]
}
```
...
- **Database**: `@alanos/plugin-sql`
- **Text Generation**: Most LLM plugins (`openai`, `anthropic`, `google-genai`, `ollama`)
- **Embeddings**: `@alanos/plugin-openai`, `@alanos/plugin-google-genai`, `@alanos/plugin-ollama`
- **Image Generation**: `@alanos/plugin-openai`, dedicated image plugins
- **Core Functionality**: `@alanos/plugin-bootstrap`
...
**For 99% of use cases, you should include `@alanos/plugin-bootstrap` in your plugins array.** Without it, your agent will appear "deaf and mute" - unable to process messages or respond to users. Think of it as the essential nervous system that connects your agent's brain to the outside world.
...
```bash
alanos project add-plugin @alanos/plugin-name
```
...
1. Add the plugin to your character.json:
   ```json
   {
     "plugins": ["@alanos/plugin-name"]
   }
   ```
2. Install the plugin: `bun install @alanos/plugin-name`
3. Rebuild: `bun build`
...
1. Use the CLI to scaffold a plugin: `alanos create` (select Plugin)
2. Implement required interfaces (actions, providers, evaluators)
3. Publish with `alanos plugins publish`
...
### How do I update alanOS after new releases?

When new versions of alanOS are released, follow these steps to update:
...
2. **Update your project dependencies:**
   ...
   This command updates all alanOS dependencies to their latest compatible versions.
...
- **Always start with `alanos dev`** after updates - it handles reinstallation and rebuilding automatically
...
2. **Check your package.json:**
   ...
   - **Important**: Don't hardcode alanOS package versions in your `package.json`
   ...
   {
     "dependencies": {
       "@alanos/core": "1.0.0",  // Will always install this exact version
       "@alanos/plugin-bootstrap": "1.0.0"
     }
   }
   ...
   {
     "dependencies": {
       "@alanos/core": "latest",  // Will update to compatible versions
       "@alanos/plugin-bootstrap": "latest"
     }
   }
   ```

3. **Common issues and solutions:**
   - **"Module not found"** - Run `alanos dev` to reinstall dependencies
...
3. **Use version control:**
   ...
   git init
   git add .
   git commit -m "Initial alanOS project"
   ```
...
1. **Use PM2 process manager with Bun:**
   ...
   pm2 start "alanos start" --name alan --interpreter bun
...
3. **Set up monitoring and automatic restarts:**
   ...
   pm2 start "alanos start" --name alan --interpreter bun --watch --max-memory-restart 2G
...
By default, the alanOS web interface runs on port 3000. If you see an error like `EADDRINUSE: address already in use :::3000`, it means another application is already using that port.

**Solution:**

1.  **Stop the Other Application**: Identify and stop the other process that is using port 3000.

2.  **Run on a Different Port**: Alternatively, you can tell alanOS to use a different port with the `--port` flag.
    ```bash
...
1.  **For PGLite (default local database)**:
    ...
    - Delete the `.alandb` folder (or the path specified in `PGLITE_DATA_DIR`). The default location is `.alan/.alandb` in your project root.
...
2. Reset your agent's memory:
   - **For PGLite**: Remove the `.alandb` folder
     ```bash
     rm -rf .alan/.alandb
     ```
...
alanos start --character ./customcharacter.json

# Multiple character files
alanos start --character ./character1.json ./character2.json

# Without .json extension (auto-added)
alanos start --character mycharacter

# From URL
alanos start --character https://example.com/mycharacter.json
```
...
2. **Backward compatibility**: Existing character files from older versions work, but you'll need to:
   ...
   - Add the appropriate plugin(s) to the `plugins` array (e.g., `@alanos/plugin-openai`)
...
alan welcomes contributions from individuals with a wide range of skills:

- **Participate in community discussions**: Share your insights, propose new ideas, and engage with other community members
- **Contribute to the development**: https://github.com/alanOS/alan
...
- **Improve the core**: Enhance the alanOS core functionality
...
- **Translation**: Help make alanOS accessible to a global audience
- **Domain Expertise**: Provide insights for specific applications of alanOS
