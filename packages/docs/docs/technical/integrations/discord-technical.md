...
Complete technical guide for integrating alanOS with Discord, including advanced features, event handling, and custom implementations.
...
```typescript
import { DiscordService } from '@alanos/plugin-discord';
...
function createRichEmbed(data: any): EmbedBuilder {
  return new EmbedBuilder()
    ...
    .setFooter({ text: 'Powered by alanOS' })
    .setTimestamp();
}
...
describe('Discord Integration', () => {
  test('processes command end-to-end', async () => {
    const response = await sendCommand('/ask What is alanOS?');

    expect(response).toMatchObject({
      success: true,
      reply: expect.stringContaining('alanOS'),
    });
  });
});
```
...
- [alanOS Discord Plugin Source](https://github.com/alanOS/alan/tree/main/packages/plugin-discord)
