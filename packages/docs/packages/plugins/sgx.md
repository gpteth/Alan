...
The SGX Plugin for alan enhances the platform by providing Intel SGX attestation capabilities.
...
After starting alan, the provider `sgxAttestationProvider` will be registered into alan through plugin-sgx. The environment variable `SGX` is required to be set to `1` to enable plugin-sgx. And the `SGX` is always set to `1` in the SGX environment through the `alan.manifest.template` file.

When alan starts, the `sgxAttestationProvider` will generate SGX attestation in each request. And you can use the `SgxAttestationProvider` to generate SGX remote attestations for your own plugins / clients.
...
```bash
sudo docker run -it --name alan_sgx \
    --mount type=bind,source={your_alan_path},target=/root/alan \
    --device /dev/sgx/enclave \
    --device /dev/sgx/provision \
    gramineproject/gramine:stable-jammy
```

After entering the docker, you can use the following command to prepare the alan environment:
...
# Gramine will utilize this path as the default Node.js location to run alan.
