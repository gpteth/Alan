...
headers = {
    ...
    "HTTP-Referer": "https://github.com/alanos/knowledge",  # Replace with your actual repo
    "X-Title": "alanOS Partner Research"
}
...
def research_partner(partner_name, partner_info):
    ...
    plugin_context = f"""
    When researching the integration, please also investigate if there might be an official alanOS plugin for {partner_name}.
    Consider looking for resources associated with alanOS plugins, such as repositories within the alanos-plugins organization on GitHub.

    If you find relevant plugin information (like its purpose from a README or description), please summarize it in the 'Integration with alan' section.
    """

    prompt = f"""
I need comprehensive, factual information about {partner_name}, who is a partner of alanOS.
...
4. ## Integration with alan
   - Specific details on how their technology integrates with alanOS
...
6. ## Market Position
   ...
   - Key partnerships besides alanOS
...
def create_brief_markdown(partner_info, research_results):
    ...
    integration_match = re.search(r'## Integration with alan\s*(.*?)(?=\n##|$)', research_results, re.DOTALL)
...
if __name__ == "__main__":
    ...
    print("Starting alanOS Partner Enhancement...")
