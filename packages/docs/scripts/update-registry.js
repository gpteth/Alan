...
/**
 * Fix image URL by removing /alan/ prefix if present
 */
function fixImageUrl(url) {
  if (!url) return null;
  return url.replace(/^\/alan\//, '/');
}
...
function fetchGithubDescription(repoPath) {
  return new Promise((resolve, reject) => {
    const options = {
      headers: {
        'User-Agent': 'alanOS-Registry-Update',
...
function processRegistryEntry(name, repoUrl) {
  ...
  const displayName = name
    .replace('@alanos-plugins/plugin-', '')
    .replace('@alanos-plugins/client-', '')
    .replace('@alanos-plugins/adapter-', '')
    .replace(/-/g, ' ')
    .split(' ')
  ...
  for (const pluginId in customData) {
    if (!registryPluginNames.includes(pluginId)) {
      ...
      missingPlugins.push({
        id: pluginId,
        name: pluginId,
        // For missing plugins, we'll use the alanos-plugins org as fallback
        repo_url: `github:alanos-plugins/${pluginId.split('/').pop()}`,
        repo_path: `alanos-plugins/${pluginId.split('/').pop()}`,
        display_name: displayName,
        type: type,
      });
    }
