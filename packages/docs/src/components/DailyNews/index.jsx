  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        let response;
        if (dateOffset === 0) {
          response = await fetch('https://m3-org.github.io/ai-news/alanos/json/daily.json');
        }

        // If we need a specific date or daily isn't available
        if (dateOffset > 0 || !response || !response.ok) {
          response = await fetch(`https://m3-org.github.io/ai-news/alanos/json/${dateStr}.json`);
          if (!response.ok) {
