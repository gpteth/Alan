...
import { IAgentRuntime, Memory, State, UUID, ChannelType, Media } from '@alanos/core';
...
describe('Choice Provider', () => {
  ...
  it('should handle errors from getTasks gracefully', async () => {
    ...
    // Cannot check logger.error calls without mocking @alanos/core
  });
});
...
describe('Settings Provider', () => {
  ...
  afterEach(async () => {
    mock.restore();
    // Cannot mock @alanos/core functions in bun:test
  });
  ...
  it('should retrieve settings in onboarding mode', async () => {
    ...
    // Note: We cannot mock findWorldsForOwner and getWorldSettings from @alanos/core
  ...
  it('should handle errors gracefully when getWorldSettings fails', async () => {
    // Note: We cannot directly mock getWorldSettings from @alanos/core in bun:test
