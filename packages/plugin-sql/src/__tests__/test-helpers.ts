import type { Plugin, UUID } from '@alanos/core';
import { AgentRuntime } from '@alanos/core';
...
export async function createTestDatabase(
  ...
  if (process.env.POSTGRES_URL) {
    ...
    // PGlite testing
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'alan-test-'));
...
export async function createIsolatedTestDatabase(
  ...
  if (process.env.POSTGRES_URL) {
    ...
    const tempDir = path.join(os.tmpdir(), `alan-test-${testId}-${Date.now()}`);
