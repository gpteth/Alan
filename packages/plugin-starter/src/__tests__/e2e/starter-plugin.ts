import { type Content, type HandlerCallback } from '@alanos/core';

/**
 * E2E (End-to-End) Test Suite for alanOS Plugins
 * ================================================
 *
 * This file contains end-to-end tests that run within a real alanOS runtime environment.
 ...
 * HOW E2E TESTS WORK:
 * -------------------
 * 1. Tests are executed by the alanOS test runner using `alanos test e2e`
 ...
 *
 * For more details, see the alanOS documentation.
 */
  ...
  tests: [
     ...
     */
    {
      ...
      fn: async (runtime) => {
        ...
        if (runtime.character.name !== 'alan') {
          throw new Error(
            `Expected character name to be "alan" but got "${runtime.character.name}"`
          );
        }
