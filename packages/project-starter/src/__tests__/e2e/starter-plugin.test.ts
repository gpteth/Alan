 ...
 *
 * This comprehensive test suite demonstrates how to write end-to-end tests for alanOS plugins.
 * These tests run in a REAL runtime environment provided by `alanos test`, meaning:
 *
  ...
  tests = [
    {
      ...
      fn: async (runtime: any) => {
        ...
        if (character.name !== 'alan') {
          throw new Error(`Expected character name to be 'alan', got '${character.name}'`);
        }
      ...
      fn: async (runtime: any) => {
        ...
        try {
          await runtime.registerPlugin({
            name: 'starter',
            description: 'A starter plugin for alan',
            init: async () => {},
            config: {},
          });
        } catch (error) {
