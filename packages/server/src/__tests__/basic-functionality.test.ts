...
describe('Basic Server Functionality', () => {
  ...
  describe('UI Enable/Disable Logic', () => {
    ...
    const testUIEnabled = (nodeEnv?: string, alanUIEnable?: string): boolean => {
      const originalNodeEnv = process.env.NODE_ENV;
      const originalUIEnable = process.env.alan_UI_ENABLE;
      ...
      if (alanUIEnable !== undefined) {
        process.env.alan_UI_ENABLE = alanUIEnable;
      } else {
        delete process.env.alan_UI_ENABLE;
      }
      ...
      if (originalUIEnable !== undefined) {
        process.env.alan_UI_ENABLE = originalUIEnable;
      } else {
        delete process.env.alan_UI_ENABLE;
      }
    ...
    it('should allow explicit override with alan_UI_ENABLE=true', () => {
      expect(testUIEnabled('production', 'true')).toBe(true);
      expect(testUIEnabled('development', 'true')).toBe(true);
      expect(testUIEnabled('production', 'TRUE')).toBe(true);
    });

    it('should allow explicit override with alan_UI_ENABLE=false', () => {
