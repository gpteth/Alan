...
describe('Client Path Resolution Logic', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `alan-test-${Date.now()}`);
  ...
  describe('Path Resolution Priority', () => {
    ...
    it('should check bun global installation path', () => {
      ...
      const bunGlobalPath = path.join(
        tempDir,
        '.bun/install/global/node_modules/@alanos/cli/dist'
      );
  ...
  describe('Fallback Paths', () => {
    it('should check common global npm paths', () => {
      const commonPaths = [
        '/usr/local/lib/node_modules/@alanos/cli/dist',
        '/usr/lib/node_modules/@alanos/cli/dist',
        path.join(os.homedir(), '.npm-global/lib/node_modules/@alanos/cli/dist'),
      ];

      // Verify these are valid path formats
      commonPaths.forEach((p) => {
        expect(path.isAbsolute(p)).toBe(true);
        expect(p).toContain('@alanos/cli/dist');
      });
    });
  ...
  describe('Development vs Production', () => {
    ...
    it('should handle require.resolve for installed packages', () => {
      ...
      const mockRequireResolve = (pkg: string) => {
        if (pkg === '@alanos/cli/package.json') {
          return path.join(tempDir, 'node_modules/@alanos/cli/package.json');
        }
        throw new Error('Module not found');
      };

      try {
        const packageJsonPath = mockRequireResolve('@alanos/cli/package.json');
        const cliPath = path.resolve(path.dirname(packageJsonPath), 'dist');

        expect(cliPath).toContain('@alanos/cli/dist');
      } catch {
