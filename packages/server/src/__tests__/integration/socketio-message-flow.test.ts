...
import type { IAgentRuntime, UUID, Character } from '@alanos/core';
import { SOCKET_MESSAGE_TYPE, ChannelType, AgentRuntime } from '@alanos/core';
import { createDatabaseAdapter } from '@alanos/plugin-sql';
    ...
    it('should establish connection and join channel', async () => {
      const connectionPromise = new Promise((resolve) => {
        client1.on('connection_established', (data) => {
          ...
          expect(data.message).toContain('Connected to alan Socket.IO server');
          resolve(data);
        });
      });
