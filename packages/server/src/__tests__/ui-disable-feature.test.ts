/**
 * Tests for alan_UI_ENABLE feature
 */

import { describe, it, expect, beforeEach, afterEach, mock, jest } from 'bun:test';

// Mock logger
mock.module('@alanos/core', async () => {
  const actual = await import('@alanos/core');
  return {
    ...actual,
    logger: {
      info: jest.fn(),
...
describe('UI Disable Feature Integration', () => {
  ...
  describe('Environment Variable Behavior', () => {
    it('should read alan_UI_ENABLE from environment', () => {
    ...
    it('should handle combined NODE_ENV and alan_UI_ENABLE scenarios', () => {
      const testScenarios = [
        // NODE_ENV, alan_UI_ENABLE, Expected UI State, Description
      ...
      testScenarios.forEach(([nodeEnv, alanUIEnable, expected, description]) => {
        const isProduction = nodeEnv === 'production';
        const uiEnabled =
          alanUIEnable !== undefined ? alanUIEnable.toLowerCase() === 'true' : !isProduction;
  ...
  describe('Documentation Consistency', () => {
    it('should match .env.example documentation patterns', () => {
      ...
      expect(envExamplePattern.variable).toBe('alan_UI_ENABLE=');
    });
  });
});
