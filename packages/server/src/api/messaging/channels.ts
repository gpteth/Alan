...
async function saveChannelUploadedFile(
  ...
  const uploadDir = path.join(process.cwd(), '.alan/data/uploads/channels', channelId);
  ...
  (router as any).post(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      const {
        ...
        source_type, // Should be something like 'alan_gui'
      } = req.body;
      ...
      try {
        ...
        const newRootMessageData = {
          ...
          metadata,
          sourceType: source_type || 'alan_gui',
        };
