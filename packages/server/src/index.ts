...
export function resolvePgliteDir(dir?: string, fallbackDir?: string): string {
  ...
  const base =
    ...
    fallbackDir ??
    path.join(process.cwd(), '.alan', '.alandb');

  // Automatically migrate legacy path (<cwd>/.alandb) to new location (<cwd>/.alan/.alandb)
  const resolved = expandTildePath(base);
  const legacyPath = path.join(process.cwd(), '.alandb');
  if (resolved === legacyPath) {
    const newPath = path.join(process.cwd(), '.alan', '.alandb');
...
export function isWebUIEnabled(): boolean {
  ...
  const uiEnabledEnv = process.env.alan_UI_ENABLE;
...
export class AgentServer {
      ...
      if (!defaultServer) {
        ...
        try {
          await (this.database as any).db.execute(`
            ...
            VALUES ('00000000-0000-0000-0000-000000000000', 'Default Server', 'alan_default', NOW(), NOW())
  ...
  private async initializeServer(options?: ServerOptions) {
    try {
      ...
      const serverAuthToken = process.env.alan_SERVER_AUTH_TOKEN;
      if (serverAuthToken) {
        ...
        logger.warn(
          'Server authentication is disabled. Set alan_SERVER_AUTH_TOKEN environment variable to enable.'
        );
      }
      ...
      if (this.isWebUIEnabled) {
        ...
        const uiEnabledEnv = process.env.alan_UI_ENABLE;
        if (uiEnabledEnv !== undefined && uiEnabledEnv.trim() !== '') {
          logger.info(`Web UI disabled by environment variable (alan_UI_ENABLE=${uiEnabledEnv})`);
        } else {
          logger.info('Web UI disabled for security (production mode)');
        }
      }

      const uploadsBasePath = path.join(process.cwd(), '.alan', 'data', 'uploads', 'agents');
      const generatedBasePath = path.join(process.cwd(), '.alan', 'data', 'generated');
      ...
      if (this.isWebUIEnabled) {
        ...
        const possiblePaths = [
          ...
          (() => {
            try {
              return path.resolve(
                path.dirname(require.resolve('@alanos/client/package.json')),
                'dist'
              );
            } catch {
              return null;
          ...
          (() => {
            try {
              ...
              const bunGlobalPath = path.join(
                os.homedir(),
                '.bun/install/global/node_modules/@alanos/server/dist/client'
              );
              ...
              try {
                ...
                if (proc.exitCode === 0 && proc.stdout) {
                  ...
                  const globalServerPath = path.join(npmRoot, '@alanos/server/dist/client');
          ...
          ...[
            '/usr/local/lib/node_modules/@alanos/server/dist/client',
            '/usr/lib/node_modules/@alanos/server/dist/client',
            path.join(os.homedir(), '.npm-global/lib/node_modules/@alanos/server/dist/client'),
                ...
                if (existsSync(nvmPath)) {
                  ...
                  for (const version of versions) {
                    const cliPath = path.join(
                      nvmPath,
                      version,
                      'lib/node_modules/@alanos/server/dist/client'
                    );
