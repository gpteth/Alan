...
# alanOS Code Quality Analysis Script
...
print_info "Starting alanOS Code Quality Analysis"
...
# Initialize report
cat > "${REPORT_FILE}" << EOF
# alanOS Code Quality Analysis Report
...
print_info "Analyzing documentation accuracy..."
{
    ...
    if [ -d "docs" ] || [ -d "packages/docs" ]; then
        ...
        grep -r "import.*from" "$DOCS_DIR" --include="*.md" 2>/dev/null | grep -E "(packages/core|@ai16z/|alanos@)" | head -10 || echo "No outdated imports found"
...
print_info "Analyzing repository standards..."
{
    ...
    echo "alanOS uses 'bun:test' exclusively. Found usage of other test frameworks:"
