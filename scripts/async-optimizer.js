...
const SKIP_PACKAGES = new Set([
  ...
  'create-alan', // Skip alan projects
  'project-starter',
  'plugin-starter',
  'plugin-example',
]);
...
async function initializeCache() {
  try {
    ...
    if (cache.version === CACHE_VERSION) {
      ...
      logVerbose(`Found ${cache.metadata.parallalanbleFunctions} parallalanble functions\n`);
    }
  } catch (error) {
...
async function updateCache(functionInfo, analysis) {
  ...
  analysisCache.metadata.parallalanbleFunctions = Object.values(analysisCache.functions).filter(
    (f) => f.analysis?.isParallalanble
  ).length;
...
Analyze this async function for parallalantion opportunities. Respond with a JSON object containing:
{
  "functionName": "${functionName}",
  "isParallalanble": boolean,
  "parallalanbleOperations": [
    {
...
\`\`\``,
    ...
    result = result.replace(/"isParallalanble"\s*:\s*"(true|false)"/g, '"isParallalanble": $1');
    ...
    if (typeof parsed.isParallalanble !== 'boolean') {
      throw new Error(
        `Invalid type for isParallalanble: expected boolean, got ${typeof parsed.isParallalanble}`
      );
    }
    if (!Array.isArray(parsed.parallalanbleOperations)) {
      throw new Error('parallalanbleOperations must be an array');
    }

    // Validate each operation in parallalanbleOperations
    parsed.parallalanbleOperations.forEach((op, index) => {
      if (!Array.isArray(op.operations)) {
        throw new Error(`operations in parallalanbleOperations[${index}] must be an array`);
      }
      if (!Array.isArray(op.lines) || op.lines.length !== 2) {
        throw new Error(`lines in parallalanbleOperations[${index}] must be an array of length 2`);
      }
      if (typeof op.type !== 'string') {
        throw new Error(`type in parallalanbleOperations[${index}] must be a string`);
      }
    });

    // Ensure optimizedCode is present if the function is parallalanble
    if (
      parsed.isParallalanble &&
      (!parsed.optimizedCode || typeof parsed.optimizedCode !== 'string')
    ) {
      console.warn(
        `[${functionName}] Warning: Function is parallalanble but no optimized code provided. Generating default optimization.`
      );
      // Generate a basic optimization if none is provided
      parsed.optimizedCode = generateDefaultOptimization(
        functionName,
        parsed.parallalanbleOperations,
        cleanedCode
      );
    }
...
async function applyOptimizations(analysisResults) {
  ...
  for (const [name, analysis] of Object.entries(analysisResults)) {
    if (!analysis.isParallalanble) {
      console.log(`Skipping ${name} - not parallalanble`);
      continue;
    }
    ...
    function visit(node) {
      ...
      try {
        if (
          ...
          const parallelOps = node.body ? analyzeASTForParallalantion(node.body, sourceCode) : [];
          ...
          asyncFunctions.push({
            ...
            loc: node.loc,
            astAnalysis: {
              isParallalanble: parallelOps.length > 0,
              parallalanbleOperations: parallelOps,
            },
          });
        }
      } catch (error) {
...
function analyzeASTForParallalantion(node, sourceCode) {
  if (!node) return [];
  ...
  // Refined blacklisted terms that prevent parallalantion
  ...
  try {
    for (let i = 0; i < awaitExpressions.length; i++) {
      ...
      if (!hasComplexDeps && !inDifferentBranches) {
        ...
        // If we have a group of parallalanble operations, add them
        const operations = [];
...
async function saveInitialCandidates(parseResults) {
  ...
  for (const fn of parseResults.asyncFunctions) {
    const candidateInfo = {
      ...
      ast_parallalanble: fn.astAnalysis.isParallalanble,
      parallalanble_functions: [],
      parallalanble_operations: [],
    };

    if (fn.astAnalysis.isParallalanble) {
      const parallelOps = fn.astAnalysis.parallalanbleOperations
      ...
      if (parallelOps.length > 0) {
        candidateInfo.parallalanble_operations = parallelOps.map((op) => ({
          lines: op.lines,
          operations: op.operations,
        ...
        candidateInfo.parallalanble_functions = Array.from(functionNames);
      }
    }
  ...
  console.log(
    `\nSaved ${candidates.length} candidates (${candidates.filter((c) => c.ast_parallalanble).length} parallalanble from AST analysis)`
  );
  return candidates;
}
...
async function saveInitialResults(parseResults) {
  ...
  for (const fn of parseResults.asyncFunctions) {
    if (fn.astAnalysis.isParallalanble) {
      const originalOps = fn.astAnalysis.parallalanbleOperations.map((op) => ({
        type: op.type,
        original_lines: `${op.lines[0]}-${op.lines[1]}`,
        sequential_calls: op.operations.map((call) => call.replace(/\(.*\)/, '()')),
        optimization: 'Potential candidate for Promise.all parallalantion',
      }));
...
async function parseAllPackages(packagesDir) {
  ...
  for (const filePath of sourceFiles) {
    try {
      ...
      const parallalanbleFns = functions.filter((fn) => fn.astAnalysis.isParallalanble);
      totalAsyncFunctions += functions.length;
      totalParallalanble += parallalanbleFns.length;
      totalProcessed++;

      if (functions.length > 0) {
        console.log(
          `${relativePath}: ${functions.length} async functions ` +
            `(${parallalanbleFns.length} potentially parallalanble)`
        );

        // Only log details for parallalanble functions if verbose logging is enabled
        if (VERBOSE_LOGGING) {
          for (const fn of parallalanbleFns) {
            console.log(`\n  Function: ${fn.name}`);
            for (const op of fn.astAnalysis.parallalanbleOperations) {
  ...
  console.log(`Potentially parallalanble: ${totalParallalanble}`);
...
async function analyzeWithAI(parseResults) {
  ...
  const analysisResults = {
    ...
    results: {},
    errors: [],
    parallalanbleFunctions: new Set(), // Track parallalanble functions
  };
  ...
  for (const fn of parseResults.asyncFunctions) {
    ...
    if (analysisCache.functions[hash]) {
      ...
      if (cachedAnalysis) {
        ...
        // Track parallalanble functions from cache
        if (cachedAnalysis.isParallalanble) {
          analysisResults.parallalanbleFunctions.add(key);
        }
  ...
  const analysisPromises = functionsToAnalyze.map((fn) =>
    queue.add(async () => {
      try {
        ...
        // Track parallalanble functions
        if (analysis.isParallalanble && analysis.parallalanbleOperations?.length > 0) {
          analysisResults.parallalanbleFunctions.add(key);
        }
        ...
        if (analysis.isParallalanble && analysis.parallalanbleOperations?.length > 0) {
          ...
          logVerbose(`[${fn.name}] Parallalanble: true`);
          logVerbose(`[${fn.name}] Parallalanble Operations:`);
          analysis.parallalanbleOperations.forEach((op) => {
  ...
  console.log(`Parallalanble functions found: ${analysisResults.parallalanbleFunctions.size}`);
  console.log(`Errors encountered: ${analysisResults.errors.length}`);

  if (analysisResults.parallalanbleFunctions.size > 0) {
    console.log('\nParallalanble Functions:');

    // Display each parallalanble function
    for (const key of analysisResults.parallalanbleFunctions) {
...
async function generateOptimizations(analysisResults) {
  ...
  // Debug: Log the parallalanble functions
  console.log('Parallalanble functions:', analysisResults.parallalanbleFunctions);

  for (const [key, analysis] of Object.entries(analysisResults.results)) {
    // Check if the function is in the parallalanbleFunctions set
    const isParallalanble = analysisResults.parallalanbleFunctions.has(key);

    // Update the analysis object with the correct isParallalanble value
    analysis.isParallalanble = isParallalanble;

    if (
      !isParallalanble ||
      !analysis.parallalanbleOperations ||
      analysis.parallalanbleOperations.length === 0
    ) {
      console.log(`Skipping ${key} - not parallalanble or no operations`);
      skippedCount++;
      continue;
    }
    ...
    try {
      ...
      finalResults.optimizations[key] = {
        ...
        operations: analysis.parallalanbleOperations,
      };
      successCount++;
  ...
  try {
    ...
    if (!candidate) {
      candidate = {
        ...
        file: fn.relativePath,
        lines,
        ast_parallalanble: fn.astAnalysis?.isParallalanble || false,
        ai_parallalanble: false,
        parallalanble_functions: [],
        parallalanble_operations: [],
      };
    ...
    if (analysis) {
      candidate.ai_parallalanble = analysis.isParallalanble;
      if (analysis.isParallalanble && analysis.parallalanbleOperations) {
        candidate.parallalanble_operations = analysis.parallalanbleOperations.map((op) => ({
        ...
        candidate.parallalanble_functions = Array.from(functionNames);
      }
    }
...
async function updateResults(fn, analysis) {
  try {
    ...
    if (analysis) {
      ...
      if (analysis.isParallalanble && analysis.parallalanbleOperations) {
        result.optimized_implementation.parallel_operations = analysis.parallalanbleOperations.map(
          (op) => ({
...
async function generateMarkdownReport(analysisResults) {
  ...
  // Add report header
  reportParts.push('# Parallalantion Optimization Report\n');
  ...
  reportParts.push(
    `- Parallalanble functions identified: ${analysisResults.parallalanbleFunctions.size}`
  );
  reportParts.push(`- Errors encountered: ${analysisResults.errors.length}\n`);

  // If no parallalanble functions were found, note that
  if (analysisResults.parallalanbleFunctions.size === 0) {
    reportParts.push('> No parallalanble functions were identified.');
    // Return early if there's nothing to report
    return reportParts.join('\n');
  }

  // Add detailed recommendations for each parallalanble function
  ...
  for (const [filePath, functions] of Object.entries(fileMap)) {
    ...
    for (const { functionName, analysis } of functions) {
      ...
      // Add the parallalanble operations
      reportParts.push('**Parallalanble Operations:**\n');

      analysis.parallalanbleOperations.forEach((op, index) => {
...
/**
 * Uses regex-based analysis to find async functions and potential parallalantion opportunities
...
async function analyzeWithRegex(filePath) {
  try {
    ...
    // Analyze each function for parallalanble operations
    for (const fn of asyncFunctions) {
      ...
      // Check for parallalantion opportunities
      let hasParallalanble = false;
      const parallelOps = [];

      for (const [calledFn, calls] of Object.entries(awaitCalls)) {
        if (calls.length > 1) {
          hasParallalanble = true;
      ...
      if (hasParallalanble) {
        ...
        // Still add to results, but mark as not parallalanble
...
async function main() {
  ...
  if (cmdArgs.analyze) {
    ...
    const analysisResults = await (async () => {
      if (cmdArgs.ai || cmdArgs['with-ai']) {
        try {
          ...
          return { results: {}, parallalanbleFunctions: new Set(), errors: [error] };
        }
      }

      // If no AI analysis, create results structure with parallalanble functions
      const result = {
        results: {},
        parallalanbleFunctions: new Set(),
        errors: [],
      };
      ...
      if (parseResults && parseResults.functions && Array.isArray(parseResults.functions)) {
        // Add all functions with regex-identified parallalantion opportunities
        parseResults.functions.forEach((fn) => {
          if (fn.canBeParallelized) {
            result.parallalanbleFunctions.add(fn.name);
            result.results[fn.name] = {
              canBeParallelized: true,
              isParallalanble: true,
              parallalanbleOperations: fn.parallalanbleOperations || [],
    ...
    console.log(
      `- Potentially parallalanble functions: ${analysisResults && analysisResults.parallalanbleFunctions ? analysisResults.parallalanbleFunctions.size : 0}`
    );
    console.log(
    ...
    console.log('- parallalanbleFunctions:', analysisResults.parallalanbleFunctions);
    ...
    if (
      shouldApply &&
      analysisResults &&
      analysisResults.parallalanbleFunctions &&
      analysisResults.parallalanbleFunctions.size > 0
    ) {
      console.log('\nApplying optimizations...');
