...
# alanOS Documentation Consistency Checker
...
# Initialize report
cat > "${REPORT_FILE}" << EOF
# alanOS Documentation Consistency Report
...
print_info "Starting alanOS Documentation Consistency Check"
...
print_section "Outdated Code Examples"
{
    ...
    grep -r "@ai16z/" "$DOCS_DIR" --include="*.md" 2>/dev/null | while read -r line; do
        ...
        echo "- $file: Contains old package name @ai16z/ (should be @alanos/)"
    done | head -10

    echo ""
    echo "### Checking for incorrect import paths..."
    # Check for imports from packages/core instead of @alanos/core
    grep -r "from ['\"]packages/core" "$DOCS_DIR" --include="*.md" 2>/dev/null | while read -r line; do
        file=$(echo "$line" | cut -d: -f1)
        echo "- $file: Imports from packages/core (should be @alanos/core)"
    done | head -10
...
print_section "Generating Summary"
{
    ...
    outdated_count=$(grep -c "outdated\|old package name\|should be @alanos" "${REPORT_FILE}" 2>/dev/null || echo 0)
    ...
    echo "2. Update outdated package names (@ai16z/ → @alanos/)"
